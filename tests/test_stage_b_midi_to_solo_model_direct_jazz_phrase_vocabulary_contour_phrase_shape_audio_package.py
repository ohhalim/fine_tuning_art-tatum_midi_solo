...
from scripts.build_stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_audio_package import (
    BOUNDARY,
    NEXT_BOUNDARY,
    StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeAudioPackageError,
...
def contour_repair(root: Path, *, quality_claim: bool = False, target_passed: bool = True) -> dict:
    midi_paths = [root / f"contour_phrase_shape_rank_{index:02d}.mid" for index in range(1, 4)]
    ...
    return {
        ...
        "repair_result": {
            ...
            "stepwise_contour_bias_reduced": target_passed,
            "no_overlap": True,
        },
        "readiness": {
            "boundary": "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_repair",
...
class StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeAudioPackageTest(unittest.TestCase):
    ...
    def test_rejects_unpassed_repair(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            renderer = root / "fluidsynth"
            soundfont = root / "piano.sf2"
            renderer.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
            soundfont.write_bytes(b"sf2")
            with self.assertRaises(StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeAudioPackageError):
                build_audio_package_report(
                    contour_repair(root, target_passed=False),
                    output_dir=root / "audio_package",
                    renderer_path=str(renderer),
                    soundfont_path=str(soundfont),
                    sample_rate=44100,
    ...
    def test_rejects_upstream_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaises(StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeAudioPackageError):
                build_audio_package_report(
                    contour_repair(root, quality_claim=True),
                    output_dir=root / "audio_package",
                    renderer_path=str(renderer),
                    soundfont_path=str(soundfont),
                    sample_rate=44100,
