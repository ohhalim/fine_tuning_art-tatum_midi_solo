...
from scripts.render_music_transformer_solo_yield_density_aftercare_audio import (
    NEXT_BOUNDARY,
    SCHEMA_VERSION,
    SoloYieldDensityAftercareAudioPackageError,
    build_audio_package,
    validate_report,
)
...
def repair_sweep(root: Path, *, quality_claim: bool = False) -> dict:
    ...
    for path in midi_paths:
        write_midi(path)
    return {
        "schema_version": DENSITY_REPAIR_SCHEMA_VERSION,
        ...
        "aggregate": {
            ...
            "low_note_count_before": 1,
            "low_note_count_after": 0,
            "inserted_note_total": 2,
...
class MusicTransformerSoloYieldDensityAftercareAudioTest(unittest.TestCase):
    def test_renders_density_repaired_midi_wavs_without_quality_claim(self) -> None:
    ...
    def test_rejects_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            renderer.write_text("#!/bin/sh\n", encoding="utf-8")
            soundfont.write_bytes(b"sf2")
            with self.assertRaises(SoloYieldDensityAftercareAudioPackageError):
                build_audio_package(
