...
from scripts.run_music_transformer_solo_yield_chord_tone_landing_repair_sweep import (
    ...
    SCHEMA_VERSION,
    SELECTED_TARGET,
    SoloYieldChordToneLandingRepairSweepError,
    build_repair_sweep,
    validate_report,
)
...
class MusicTransformerSoloYieldChordToneLandingRepairSweepTest(unittest.TestCase):
    def test_repairs_final_landing_without_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            package = package_report(root)
            objective = build_objective_failure_review(package, output_dir=root / "objective")
        ...
        self.assertEqual(summary["repaired_midi_count"], 3)
        self.assertEqual(summary["final_landing_not_chord_tone_count_before"], 3)
        self.assertEqual(summary["final_landing_not_chord_tone_count_after"], 0)
        self.assertEqual(summary["changed_note_total"], 3)
    ...
    def test_rejects_quality_claim_in_package(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            package = package_report(root, quality_claim=True)
            objective = build_objective_failure_review(package_report(root), output_dir=root / "objective")
            with self.assertRaises(SoloYieldChordToneLandingRepairSweepError):
                build_repair_sweep(
                    package_report=package,
                    objective_review=objective,
                    output_dir=root / "repair",
    ...
    def test_rejects_non_landing_target(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            package = package_report(root)
            objective = build_objective_failure_review(package, output_dir=root / "objective")
            objective["decision"]["selected_next_target"] = "dead_air_aftercare"
            with self.assertRaises(SoloYieldChordToneLandingRepairSweepError):
                build_repair_sweep(
                    package_report=package,
                    objective_review=objective,
