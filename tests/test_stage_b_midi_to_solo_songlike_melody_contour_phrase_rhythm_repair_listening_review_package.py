...
class StageBMidiToSoloSonglikeMelodyContourPhraseRhythmRepairListeningReviewPackageTest(unittest.TestCase):
    def test_builds_pending_listening_review_package_without_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(report["issue_number"], 1200)
    ...
    def test_rejects_source_context_delta_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = audio_package_report(root)
            source["summary"]["source_outside_soloing_repair_source_pitch_role_risk_delta"] = 1
    ...
    def test_rejects_source_context_preservation_flag_false(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = audio_package_report(root)
            source["summary"][BRIDGE_SOURCE_CONTEXT_PRESERVED_KEYS[0]] = False
    ...
    def test_constants_are_stable(self) -> None:
        ...
        self.assertEqual(
            SCHEMA_VERSION,
            "stage_b_midi_to_solo_songlike_melody_contour_phrase_rhythm_repair_listening_review_package_v5",
        )
