...
class StageBMidiToSoloSonglikeMelodyContourRepairListeningInputGuardTest(unittest.TestCase):
    def test_blocks_preference_fill_when_review_input_pending(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(report["issue_number"], 1190)
    ...
    def test_rejects_source_schema_version_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = source_package()
            source["source_schema_versions"]["songlike_melody_contour_repair_audio_package"] = "old"
            with self.assertRaises(
                StageBMidiToSoloSonglikeMelodyContourRepairListeningInputGuardError
            ):
                build_listening_review_input_guard_report(
    ...
    def test_rejects_false_schema_context_preserved(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            source = source_package()
            source["source_summary"]["source_outside_soloing_repair_schema_context_preserved"] = False
            with self.assertRaises(
                StageBMidiToSoloSonglikeMelodyContourRepairListeningInputGuardError
            ):
                build_listening_review_input_guard_report(
    ...
    def test_constants_are_stable(self) -> None:
        ...
        self.assertEqual(
            SCHEMA_VERSION,
            "stage_b_midi_to_solo_songlike_melody_contour_repair_listening_review_input_guard_v5",
        )
