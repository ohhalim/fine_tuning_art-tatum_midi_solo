...
def source_report(root: Path, *, quality_claim: bool = False) -> dict:
    ...
    for index in range(1, 7):
        ...
        write_midi(midi_path, [60, 62, 64, 67] * 6)
        repairs.append(
            {
                "source": "unit_source",
                "rank": index,
                ...
                "source_failure_label_count": 2,
                "changed_pitch_count": index,
                "changed_time_count": 20 + index,
                "note_count": 24,
                "repaired_labeling": {
                    "failure_labels": ["songlike_melody_not_soloing"] if index < 6 else [],
                    "metrics": {
                        "note_count": 24,
...
class StageBMidiToSoloTargetedQualityRepairAudioTest(unittest.TestCase):
    def test_renders_targeted_repair_wavs_without_quality_claim(self) -> None:
