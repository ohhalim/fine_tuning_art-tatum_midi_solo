...
from scripts.guard_music_transformer_solo_yield_chord_tone_landing_listening_input import (
    ...
    SoloYieldChordToneLandingListeningInputGuardError,
    build_guard_report,
    validate_report,
)
...
class MusicTransformerSoloYieldChordToneLandingListeningInputGuardTest(unittest.TestCase):
    ...
    def test_rejects_source_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            with self.assertRaises(SoloYieldChordToneLandingListeningInputGuardError):
                build_guard_report(
