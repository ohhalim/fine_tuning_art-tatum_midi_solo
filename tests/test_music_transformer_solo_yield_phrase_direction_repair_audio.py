...
from scripts.render_music_transformer_solo_yield_phrase_direction_repair_audio import (
    NEXT_BOUNDARY,
    SCHEMA_VERSION,
    SoloYieldPhraseDirectionRepairAudioPackageError,
    build_audio_package,
    validate_report,
)
...
def repair_sweep(root: Path, *, quality_claim: bool = False) -> dict:
    ...
    for path in midi_paths:
        write_midi(path)
    return {
        "schema_version": PHRASE_DIRECTION_REPAIR_SCHEMA_VERSION,
        ...
        "aggregate": {
            ...
            "weak_direction_change_count_before": 2,
...
class MusicTransformerSoloYieldPhraseDirectionRepairAudioTest(unittest.TestCase):
    def test_renders_phrase_direction_repaired_midi_wavs_without_quality_claim(self) -> None:
    ...
    def test_rejects_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            renderer.write_text("#!/bin/sh\n", encoding="utf-8")
            soundfont.write_bytes(b"sf2")
            with self.assertRaises(SoloYieldPhraseDirectionRepairAudioPackageError):
                build_audio_package(
