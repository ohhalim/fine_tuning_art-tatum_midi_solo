...
from scripts.build_stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_listening_review import (
    BOUNDARY,
    NEXT_BOUNDARY,
    StageBMidiToSoloControlledScaleCheckpointTemperatureGuardListeningReviewError,
...
def audio_package(root: Path, *, quality_claim: bool = False, technical_valid: bool = True) -> dict:
    ...
    for rank, seed in enumerate([46, 52, 61], start=1):
        wav_path = root / f"temperature_guard_rank_{rank:02d}_seed_{seed}.wav"
        fake_wav(wav_path)
        rendered.append(
            {
                "rank": rank,
                "seed": seed,
                "sample_index": rank,
                "midi_path": str(root / f"temperature_guard_rank_{rank:02d}.mid"),
    ...
    return {
        "schema_version": "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_audio_review_package_v1",
        "audio_package_boundary": {
            "boundary": AUDIO_BOUNDARY,
            "source_boundary": "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_repair_consolidation",
            "candidate_count": 3,
...
class StageBMidiToSoloControlledScaleCheckpointDeadAirRepeatabilityTemperatureGuardListeningReviewTest(
    unittest.TestCase
):
