...
from scripts.build_music_transformer_solo_yield_interval_contour_aftercare_listening_package import (
    ...
    NEXT_BOUNDARY,
    SCHEMA_VERSION,
    SoloYieldIntervalContourAftercareListeningPackageError,
...
def audio_package(root: Path, *, quality_claim: bool = False) -> dict:
    ...
    for midi_path, wav_path in zip(midi_paths, wav_paths):
        ...
        write_wav(wav_path)
    return {
        "schema_version": INTERVAL_AUDIO_PACKAGE_SCHEMA_VERSION,
...
class MusicTransformerSoloYieldIntervalContourAftercareListeningPackageTest(unittest.TestCase):
    ...
    def test_rejects_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            with self.assertRaises(SoloYieldIntervalContourAftercareListeningPackageError):
                build_listening_package(
