...
- model-first path가 최소 1개 seed에서 fallback 없이 완료된다.
- result JSON has `fallback_used=false`.
- result JSON has `model_repaired=true`.
- pitch range가 21~108 안에 있다.
- duration이 요청 bars에 맞게 trim된다.

### Gate C: Optional FastAPI
...
- [ ] result path points to a MIDI file.
