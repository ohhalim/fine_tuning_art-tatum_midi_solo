"""Build WAV review package for jazz phrase vocabulary repair probe candidates."""
...
class StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError(ValueError):
    pass


BOUNDARY = "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_repair_audio_package"
NEXT_BOUNDARY = "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_repair_listening_review"
...
def read_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError(f"report missing: {path}")
...
def validate_repair_probe(probe_report: dict[str, Any], expected_count: int) -> list[dict[str, Any]]:
    readiness = _dict(probe_report.get("readiness"))
    decision = _dict(probe_report.get("decision"))
    repair_result = _dict(probe_report.get("repair_result"))
    analysis_by_rank = {
        _int(_dict(item).get("rank")): _dict(item)
        for item in _list(probe_report.get("candidate_analyses"))
        if _int(_dict(item).get("rank")) > 0
    }
    if str(probe_report.get("boundary") or readiness.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError(
            "repair probe boundary required"
        )
    if str(decision.get("next_boundary") or "") != BOUNDARY:
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError(
            "repair probe must route to audio package"
        )
    if not bool(readiness.get("repair_probe_completed", False)):
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError("repair probe completion required")
    if not bool(repair_result.get("target_passed", False)):
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError("repair target pass required")
    blocked_claims = [
    ...
    for item in _list(probe_report.get("generated_candidates"))[: int(expected_count)]:
        row = _dict(item)
        ...
        candidates.append(
            {
                ...
                "duration_most_common_ratio": _float(analysis.get("duration_most_common_ratio")),
                "ioi_most_common_ratio": _float(analysis.get("ioi_most_common_ratio")),
                "analysis_flags": _list(analysis.get("analysis_flags")),
            }
        )
    if len(candidates) != int(expected_count):
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError("unexpected candidate count")
    return candidates
...
def build_render_plan(
    ...
    *,
    ...
    if not renderer.exists():
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError(f"renderer not found: {renderer}")
    ...
    for item in candidates:
        wav_path = output_dir / "audio" / f"jazz_phrase_repair_rank_{int(item['rank']):02d}.wav"
        plan.append(
            {
                **item,
        ...
        rendered.append(
            {
                ...
                "duration_most_common_ratio": item["duration_most_common_ratio"],
                "ioi_most_common_ratio": item["ioi_most_common_ratio"],
...
def build_repair_audio_package_report(
    ...
    *,
    ...
    candidates = validate_repair_probe(probe_report, expected_count=expected_file_count)
    ...
    return {
        ...
        "decision": {
            ...
            "reason": "repaired MIDI candidates rendered to WAV for listening review without quality claim",
        },
        "not_proven": [
        ...
        "next_recommended_issue": "Stage B MIDI-to-solo model-direct jazz phrase vocabulary repair listening review",
    }


def validate_repair_audio_package_report(
    report: dict[str, Any],
    *,
    ...
    if len(files) != int(expected_file_count):
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError("unexpected rendered file count")
    for item in files:
        wav_file = _dict(_dict(item).get("wav_file"))
        if not bool(wav_file.get("exists", False)):
            raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError("missing rendered WAV")
        if _int(wav_file.get("sample_rate")) != int(expected_sample_rate):
            raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError("unexpected sample rate")
        if _int(wav_file.get("frame_count")) <= 0:
            raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError("empty WAV")
        if _int(wav_file.get("size_bytes")) <= 44:
            raise StageBMidiToSoloModelDirectJazzPhraseVocabularyRepairAudioPackageError("invalid WAV size")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build repaired jazz phrase WAV package")
    parser.add_argument("--repair_probe", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_repair_audio_package",
    )
...
def main() -> int:
    ...
    write_json(output_dir / "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_repair_audio_package.json", report)
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_repair_audio_package_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(output_dir / "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_repair_audio_package.md", markdown)
    if args.doc_path:
