"""Build a listening review package for repaired phrase-bank MIDI-to-solo output."""
...
class StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningPackageError(ValueError):
    pass


BOUNDARY = "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_package"
NEXT_BOUNDARY = "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_input_guard"
SCHEMA_VERSION = "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_package_v1"
...
def _require_no_quality_claim(container: dict[str, Any], *, label: str) -> None:
    ...
    if claimed:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningPackageError(
...
def validate_audio_render_report(report: dict[str, Any], *, expected_count: int) -> list[dict[str, Any]]:
    ...
    if str(boundary.get("boundary") or "") != AUDIO_RENDER_BOUNDARY:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningPackageError(
            "repaired audio render boundary required"
        )
    if str(decision.get("next_boundary") or "") != AUDIO_RENDER_NEXT_BOUNDARY:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningPackageError(
            "audio render report must route to repaired listening review"
        )
    required_true = [
    ...
    *,
    ...
    return {
        ...
        "decision": {
            ...
            "reason": "repaired WAV/MIDI review items are packaged; preference remains pending until validated listening input",
        },
        "not_proven": [
        ...
        "next_recommended_issue": "Stage B MIDI-to-solo phrase-bank dead-air density repair listening review input guard",
    }
...
def validate_listening_review_package_report(
    ...
    *,
    ...
    if expected_next_boundary and str(decision.get("next_boundary") or "") != expected_next_boundary:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningPackageError("unexpected next boundary")
    ...
    if _int(readiness.get("review_item_count")) != int(expected_review_item_count):
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningPackageError("review item count mismatch")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    for item in report["review_items"]:
        lines.append(
            ...
            f"MIDI `{item['midi_path']}`, WAV `{item['wav_path']}`, "
            f"dead-air `{item['source_dead_air_ratio']:.4f}`, notes `{item['source_note_count']}`"
        )
    lines.extend(
        [
            "",
    ...
    for item in package["required_input_fields"]:
        lines.append(f"- `{item}`")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build repaired phrase-bank listening review package")
    parser.add_argument("--audio_render_report", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_package",
    )
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--doc_path", type=str, default="")
    parser.add_argument("--issue_number", type=int, default=646)
...
def main() -> int:
    ...
    write_json(
        output_dir / "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_package.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_package_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir / "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_package.md",
        markdown,
    )
    if args.doc_path:
