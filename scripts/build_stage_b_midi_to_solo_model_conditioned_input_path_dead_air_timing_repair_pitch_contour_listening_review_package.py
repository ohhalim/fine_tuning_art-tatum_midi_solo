"""Build a listening review package for pitch-contour repaired MIDI-to-solo output."""
...
from scripts.render_stage_b_midi_to_solo_model_conditioned_input_path_dead_air_timing_repair_pitch_contour_audio import (  # noqa: E402
    BOUNDARY as SOURCE_BOUNDARY,
    NEXT_BOUNDARY as SOURCE_NEXT_BOUNDARY,
)


class StageBMidiToSoloModelConditionedInputPathDeadAirTimingRepairPitchContourListeningReviewPackageError(
    ValueError
):
    pass
...
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_model_conditioned_input_path_"
    "dead_air_timing_repair_pitch_contour_listening_review_package_v1"
)
...
def _require_no_quality_claim(container: dict[str, Any], *, label: str) -> None:
    ...
    if claimed:
        raise StageBMidiToSoloModelConditionedInputPathDeadAirTimingRepairPitchContourListeningReviewPackageError(
...
def validate_audio_package_report(
    ...
    *,
    ...
    if str(boundary.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloModelConditionedInputPathDeadAirTimingRepairPitchContourListeningReviewPackageError(
            "pitch-contour audio package boundary required"
        )
    ...
    required_true = [
        ...
        "pitch_contour_audio_package_completed",
    ]
    ...
    *,
    ...
    return {
        ...
        "decision": {
            ...
            "reason": "pitch-contour WAV review items are packaged; preference remains pending until validated listening input",
        },
        "not_proven": [
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build pitch-contour listening review package"
    )
    ...
    parser.add_argument("--issue_number", type=int, default=702)
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_input_path_dead_air_timing_repair_pitch_contour_listening_review_package.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_input_path_dead_air_timing_repair_pitch_contour_listening_review_package_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_input_path_dead_air_timing_repair_pitch_contour_listening_review_package.md",
        markdown,
    )
    if args.doc_path:
