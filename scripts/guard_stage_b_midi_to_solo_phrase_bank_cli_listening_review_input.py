"""Guard CLI phrase-bank listening preference fill against missing review input."""
...
from scripts.build_stage_b_midi_to_solo_phrase_bank_cli_listening_review_package import (  # noqa: E402
    BOUNDARY as SOURCE_BOUNDARY,
    NEXT_BOUNDARY as SOURCE_NEXT_BOUNDARY,
)


class StageBMidiToSoloPhraseBankCliListeningInputGuardError(ValueError):
    pass


BOUNDARY = "stage_b_midi_to_solo_phrase_bank_cli_listening_review_input_guard"
FILL_BOUNDARY = "stage_b_midi_to_solo_phrase_bank_cli_listening_review_fill"
OBJECTIVE_NEXT_BOUNDARY = "stage_b_midi_to_solo_phrase_bank_cli_objective_only_next_decision"
SCHEMA_VERSION = "stage_b_midi_to_solo_phrase_bank_cli_listening_review_input_guard_v1"
...
def _require_no_quality_claim(container: dict[str, Any], *, label: str) -> None:
    ...
    if claimed:
        raise StageBMidiToSoloPhraseBankCliListeningInputGuardError(
...
def validate_source_package(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(report.get("boundary") or readiness.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloPhraseBankCliListeningInputGuardError(
            "CLI listening review package boundary required"
        )
    if str(decision.get("next_boundary") or "") != SOURCE_NEXT_BOUNDARY:
        raise StageBMidiToSoloPhraseBankCliListeningInputGuardError(
            "CLI review package must route to input guard"
        )
    ...
    if _int(readiness.get("review_item_count")) <= 0:
        raise StageBMidiToSoloPhraseBankCliListeningInputGuardError("review items required")
    if bool(decision.get("critical_user_input_required", True)):
        raise StageBMidiToSoloPhraseBankCliListeningInputGuardError(
            "critical user input should not be required"
        )
    _require_no_quality_claim(readiness, label="CLI listening review package readiness")
    return {
    ...
    *,
    ...
    return {
        ...
        "next_recommended_issue": (
            "Stage B MIDI-to-solo phrase-bank CLI listening review fill"
            if validated_input
            else "Stage B MIDI-to-solo phrase-bank CLI objective-only next decision"
        ),
    }
...
def validate_listening_review_input_guard_report(
    ...
    *,
    ...
    if expected_next_boundary and str(decision.get("next_boundary") or "") != expected_next_boundary:
        raise StageBMidiToSoloPhraseBankCliListeningInputGuardError("unexpected next boundary")
    if require_guard_completed and not bool(readiness.get("listening_review_input_guard_completed", False)):
        raise StageBMidiToSoloPhraseBankCliListeningInputGuardError("guard completion required")
    if require_pending_input and bool(guard.get("validated_review_input_present", True)):
        raise StageBMidiToSoloPhraseBankCliListeningInputGuardError("review input should remain pending")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Guard CLI phrase-bank listening review input")
    parser.add_argument("--source_package", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_phrase_bank_cli_listening_review_input_guard",
    )
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--doc_path", type=str, default="")
    parser.add_argument("--issue_number", type=int, default=660)
...
def main() -> int:
    ...
    write_json(output_dir / "stage_b_midi_to_solo_phrase_bank_cli_listening_review_input_guard.json", report)
    write_json(
        output_dir / "stage_b_midi_to_solo_phrase_bank_cli_listening_review_input_guard_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(output_dir / "stage_b_midi_to_solo_phrase_bank_cli_listening_review_input_guard.md", markdown)
    if args.doc_path:
