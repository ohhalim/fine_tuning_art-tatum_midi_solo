"""Decide next repair target after density-aftercare repair from objective evidence."""
...
from scripts.guard_music_transformer_solo_yield_density_aftercare_listening_input import (  # noqa: E402
...
SCHEMA_VERSION = "music_transformer_solo_yield_density_aftercare_objective_next_v1"
BOUNDARY = "music_transformer_solo_yield_density_aftercare_objective_only_next_decision"
...
NEXT_BOUNDARY_LISTENING_REVIEW = "music_transformer_solo_yield_density_aftercare_listening_review"
NEXT_BOUNDARY_LISTENING_REVIEW_FILL = (
    "music_transformer_solo_yield_density_aftercare_listening_review_fill"
)


class SoloYieldDensityAftercareObjectiveNextError(ValueError):
    pass
...
def read_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise SoloYieldDensityAftercareObjectiveNextError(f"json not found: {path}")
...
def _require_no_quality_claim(readiness: dict[str, Any]) -> None:
    ...
    if claimed:
        raise SoloYieldDensityAftercareObjectiveNextError(f"unexpected quality claim: {claimed}")
...
def validate_guard_report(guard_report: dict[str, Any]) -> dict[str, Any]:
    if str(guard_report.get("schema_version") or "") != INPUT_GUARD_SCHEMA_VERSION:
        raise SoloYieldDensityAftercareObjectiveNextError("input guard schema required")
...
def validate_repair_sweep(repair_sweep: dict[str, Any]) -> list[dict[str, Any]]:
    if str(repair_sweep.get("schema_version") or "") != REPAIR_SWEEP_SCHEMA_VERSION:
        raise SoloYieldDensityAftercareObjectiveNextError("repair sweep schema required")
    ...
    if not bool(aggregate.get("target_supported", False)):
        raise SoloYieldDensityAftercareObjectiveNextError("completed density aftercare required")
    if _int(aggregate.get("low_note_count_after")) != 0:
        raise SoloYieldDensityAftercareObjectiveNextError("low note-count residual risk must be zero")
    if _int(aggregate.get("chord_tone_ratio_decrease_count")) != 0:
        raise SoloYieldDensityAftercareObjectiveNextError("chord-tone ratio decrease must be zero")
    if _int(aggregate.get("weak_direction_change_count_after")) != 0:
        raise SoloYieldDensityAftercareObjectiveNextError("weak direction residual risk must be zero")
    if _int(aggregate.get("final_landing_not_chord_tone_count_after")) != 0:
        raise SoloYieldDensityAftercareObjectiveNextError("final landing residual risk must be zero")
    rows = [_dict(row) for row in _list(repair_sweep.get("candidate_repairs"))]
    if not rows:
        raise SoloYieldDensityAftercareObjectiveNextError("repair rows required")
    return rows
...
def select_next_target(aggregate: dict[str, Any]) -> tuple[str, str, str]:
    ...
    if low_chord > 0:
        return (
            ...
            "low MIDI chord-tone ratio remains after density aftercare",
        )
    ...
    if wide_interval > 0:
        return (
            ...
            "wide interval review count remains after density aftercare",
        )
    return (
...
def build_decision(
    ...
    *,
    ...
    write_json(output_dir / "density_aftercare_objective_next_decision.json", report)
    write_json(
        output_dir / "density_aftercare_objective_next_decision_summary.json",
        validate_report(report, require_no_quality_claim=True),
    )
    write_text(output_dir / "density_aftercare_objective_next_decision.md", markdown_report(report))
    return report


def validate_report(report: dict[str, Any], *, require_no_quality_claim: bool) -> dict[str, Any]:
    if str(report.get("schema_version") or "") != SCHEMA_VERSION:
        raise SoloYieldDensityAftercareObjectiveNextError("schema version mismatch")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Music Transformer Solo Yield Density Aftercare Objective-Only Next Decision",
        "",
        "## Summary",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Decide objective next target after density aftercare")
    parser.add_argument(
        "--repair_sweep_report",
        type=str,
        default=(
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "solo_yield_density_aftercare_listening_input_guard/"
            "issue_1300_density_aftercare_input_guard/"
            "density_aftercare_listening_input_guard.json"
        ),
    )
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/music_transformer_finetune_mvp/solo_yield_density_aftercare_objective_next",
    )
