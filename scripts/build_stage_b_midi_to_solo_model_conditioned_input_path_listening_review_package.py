"""Build a listening review package for model-conditioned MIDI-to-solo output."""
...
from scripts.consolidate_stage_b_midi_to_solo_model_conditioned_input_path_replacement import (  # noqa: E402
...
class StageBMidiToSoloModelConditionedInputPathListeningReviewPackageError(ValueError):
    pass


BOUNDARY = "stage_b_midi_to_solo_model_conditioned_input_path_listening_review_package"
NEXT_BOUNDARY = "stage_b_midi_to_solo_model_conditioned_input_path_listening_review_input_guard"
SCHEMA_VERSION = "stage_b_midi_to_solo_model_conditioned_input_path_listening_review_package_v1"
...
def validate_replacement_report(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(decision.get("next_boundary") or "") != REPLACEMENT_NEXT_BOUNDARY:
        raise StageBMidiToSoloModelConditionedInputPathListeningReviewPackageError(
            "replacement report must route to listening review package"
        )
    required_true = [
        ...
        "listening_review_package_required",
    ]
    missing = [name for name in required_true if not bool(readiness.get(name, False))]
    if missing:
        raise StageBMidiToSoloModelConditionedInputPathListeningReviewPackageError(
            f"missing replacement readiness: {missing}"
        )
    _require_no_quality_claim(readiness, label="replacement readiness")
    return {
...
def validate_audio_render_report(report: dict[str, Any], *, expected_count: int) -> list[dict[str, Any]]:
    ...
    if len(items) < int(expected_count):
        raise StageBMidiToSoloModelConditionedInputPathListeningReviewPackageError(
            "rendered audio file count below expected"
        )
        ...
        review_items.append(
            {
                ...
                "source_chord_tone_ratio": _float(item.get("source_chord_tone_ratio")),
...
def build_listening_review_package_report(
    *,
    ...
    replacement = validate_replacement_report(replacement_report)
    ...
    return {
        ...
        "boundary": BOUNDARY,
        "source_boundaries": {
            "replacement_consolidation": REPLACEMENT_BOUNDARY,
        ...
        "decision": {
            ...
            "reason": "ranked WAV review items are packaged; preference remains pending until validated listening input",
        },
        "not_proven": [
        ...
        "next_recommended_issue": "Stage B MIDI-to-solo model-conditioned input path listening review input guard",
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build model-conditioned input-path listening review package")
    parser.add_argument("--replacement_report", type=str, required=True)
    parser.add_argument("--audio_render_report", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_model_conditioned_input_path_listening_review_package",
    )
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--doc_path", type=str, default="")
    parser.add_argument("--issue_number", type=int, default=630)
...
def main() -> int:
    ...
    report = build_listening_review_package_report(
        replacement_report=read_json(Path(args.replacement_report)),
    ...
    write_json(output_dir / "stage_b_midi_to_solo_model_conditioned_input_path_listening_review_package.json", report)
    write_json(
        output_dir / "stage_b_midi_to_solo_model_conditioned_input_path_listening_review_package_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(output_dir / "stage_b_midi_to_solo_model_conditioned_input_path_listening_review_package.md", markdown)
    if args.doc_path:
