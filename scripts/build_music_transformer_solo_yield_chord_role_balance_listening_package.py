"""Build listening package for chord-role-balance repaired solo-yield candidates."""
...
from scripts.render_music_transformer_solo_yield_chord_role_balance_repair_audio import (  # noqa: E402
...
SCHEMA_VERSION = "music_transformer_solo_yield_chord_role_balance_listening_package_v1"
INPUT_SCHEMA_VERSION = "music_transformer_solo_yield_chord_role_balance_listening_input_v1"
BOUNDARY = "music_transformer_solo_yield_chord_role_balance_repair_listening_package"
NEXT_BOUNDARY = "music_transformer_solo_yield_chord_role_balance_repair_listening_input_guard"


class SoloYieldChordRoleBalanceListeningPackageError(ValueError):
    pass
...
def _require_no_quality_claim(report: dict[str, Any]) -> None:
    ...
    if claimed:
        raise SoloYieldChordRoleBalanceListeningPackageError(f"unexpected quality claim: {claimed}")
...
def _validate_audio_package(report: dict[str, Any], *, min_candidates: int) -> list[dict[str, Any]]:
    if str(report.get("schema_version") or "") != AUDIO_PACKAGE_SCHEMA_VERSION:
        raise SoloYieldChordRoleBalanceListeningPackageError("audio package schema required")
    _require_no_quality_claim(report)
    aggregate = _dict(report.get("aggregate"))
    if not bool(aggregate.get("technical_wav_validation", False)):
        raise SoloYieldChordRoleBalanceListeningPackageError("technical WAV validation required")
    rows = [_dict(item) for item in _list(report.get("rendered_audio_files"))]
    if len(rows) < int(min_candidates):
        raise SoloYieldChordRoleBalanceListeningPackageError("rendered audio file count below requirement")
    for row in rows:
        midi_path = Path(str(row.get("repaired_midi_path") or ""))
        wav_path = Path(str(_dict(row.get("wav_file")).get("path") or ""))
        if not midi_path.exists():
            raise SoloYieldChordRoleBalanceListeningPackageError(f"MIDI missing: {midi_path}")
        if not wav_path.exists():
            raise SoloYieldChordRoleBalanceListeningPackageError(f"WAV missing: {wav_path}")
    return rows
...
def copy_candidates(audio_rows: list[dict[str, Any]], *, output_dir: Path) -> list[dict[str, Any]]:
    ...
    for row in audio_rows:
        ...
        target_stem = f"candidate_{review_index:02d}_{case_label}_chord_role_balance_repair"
    ...
    *,
    ...
    report = {
        ...
        "decision": {
            ...
            "selected_next_target": "chord_role_balance_repair_listening_input_guard",
...
def validate_report(report: dict[str, Any], *, min_candidates: int = 1) -> dict[str, Any]:
    if str(report.get("schema_version") or "") != SCHEMA_VERSION:
        raise SoloYieldChordRoleBalanceListeningPackageError("schema version mismatch")
    readiness = _dict(report.get("readiness"))
    candidate_count = _int(report.get("candidate_count"))
    if candidate_count < int(min_candidates):
        raise SoloYieldChordRoleBalanceListeningPackageError("candidate count below requirement")
    if not bool(readiness.get("listening_package_ready", False)):
        raise SoloYieldChordRoleBalanceListeningPackageError("listening package readiness required")
    _require_no_quality_claim(report)
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Music Transformer Solo Yield Chord Role Balance Repair Listening Package",
        "",
        "## Summary",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build chord role balance repair listening package")
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/music_transformer_finetune_mvp/solo_yield_chord_role_balance_repair_listening_review",
    )
