"""Decide next repair target after phrase-direction repair from objective evidence."""
...
from scripts.guard_music_transformer_solo_yield_phrase_direction_listening_input import (  # noqa: E402
...
SCHEMA_VERSION = "music_transformer_solo_yield_phrase_direction_objective_next_v1"
BOUNDARY = "music_transformer_solo_yield_phrase_direction_repair_objective_only_next_decision"
...
NEXT_BOUNDARY_LISTENING_REVIEW = "music_transformer_solo_yield_phrase_direction_repair_listening_review"
NEXT_BOUNDARY_LISTENING_REVIEW_FILL = (
    "music_transformer_solo_yield_phrase_direction_repair_listening_review_fill"
)


class SoloYieldPhraseDirectionObjectiveNextError(ValueError):
    pass
...
def read_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise SoloYieldPhraseDirectionObjectiveNextError(f"json not found: {path}")
...
def _require_no_quality_claim(readiness: dict[str, Any]) -> None:
    ...
    if claimed:
        raise SoloYieldPhraseDirectionObjectiveNextError(f"unexpected quality claim: {claimed}")
...
def validate_guard_report(guard_report: dict[str, Any]) -> dict[str, Any]:
    if str(guard_report.get("schema_version") or "") != INPUT_GUARD_SCHEMA_VERSION:
        raise SoloYieldPhraseDirectionObjectiveNextError("input guard schema required")
...
def validate_repair_sweep(repair_sweep: dict[str, Any]) -> list[dict[str, Any]]:
    if str(repair_sweep.get("schema_version") or "") != REPAIR_SWEEP_SCHEMA_VERSION:
        raise SoloYieldPhraseDirectionObjectiveNextError("repair sweep schema required")
    ...
    if not bool(aggregate.get("target_supported", False)):
        raise SoloYieldPhraseDirectionObjectiveNextError("completed phrase direction repair required")
    if _int(aggregate.get("weak_direction_change_count_after")) != 0:
        raise SoloYieldPhraseDirectionObjectiveNextError("weak direction residual risk must be zero")
    if _int(aggregate.get("final_landing_not_chord_tone_count_after")) != 0:
        raise SoloYieldPhraseDirectionObjectiveNextError("final landing residual risk must be zero")
    rows = [_dict(row) for row in _list(repair_sweep.get("candidate_repairs"))]
    if not rows:
        raise SoloYieldPhraseDirectionObjectiveNextError("repair rows required")
    return rows
...
def select_next_target(aggregate: dict[str, Any]) -> tuple[str, str, str]:
    weak_direction = _int(aggregate.get("weak_direction_change_count"))
    ...
    if weak_direction > 0:
        return (
            ...
            BOUNDARY,
            "weak direction-change residual remains after phrase direction repair",
        )
    if low_chord > 0 and low_chord >= max(low_note, wide_interval, dead_air):
        return (
            "chord_role_balance_repair",
            NEXT_BOUNDARY_CHORD_ROLE_BALANCE,
            "low MIDI chord-tone ratio is the largest remaining objective residual",
        )
    if wide_interval > 0 and wide_interval >= max(low_note, dead_air):
        return (
            "interval_contour_aftercare",
            NEXT_BOUNDARY_INTERVAL_CONTOUR,
            "wide interval review count remains after phrase direction repair",
        )
    if low_note > 0:
        return (
            "density_aftercare",
            NEXT_BOUNDARY_DENSITY_AFTERCARE,
            "low note-count candidates remain after phrase direction repair",
        )
    return (
...
def build_decision(
    ...
    *,
    ...
    write_json(output_dir / "phrase_direction_objective_next_decision.json", report)
    write_json(
        output_dir / "phrase_direction_objective_next_decision_summary.json",
        validate_report(report, require_no_quality_claim=True),
    )
    write_text(output_dir / "phrase_direction_objective_next_decision.md", markdown_report(report))
    return report


def validate_report(report: dict[str, Any], *, require_no_quality_claim: bool) -> dict[str, Any]:
    if str(report.get("schema_version") or "") != SCHEMA_VERSION:
        raise SoloYieldPhraseDirectionObjectiveNextError("schema version mismatch")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Music Transformer Solo Yield Phrase Direction Repair Objective-Only Next Decision",
        "",
        "## Summary",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Decide objective next target after phrase direction repair")
    parser.add_argument(
        "--repair_sweep_report",
        type=str,
        default=(
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "solo_yield_phrase_direction_repair_listening_input_guard/"
            "issue_1280_phrase_direction_input_guard/"
            "phrase_direction_repair_listening_input_guard.json"
        ),
    )
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/music_transformer_finetune_mvp/solo_yield_phrase_direction_repair_objective_next",
    )
