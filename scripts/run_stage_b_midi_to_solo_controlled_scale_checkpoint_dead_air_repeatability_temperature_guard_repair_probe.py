"""Run lower-temperature guard repair probe for controlled dead-air repeatability."""
...
from scripts.decide_stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard import (  # noqa: E402
    BOUNDARY as SOURCE_BOUNDARY,
)
from scripts.run_stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repair_repeatability_probe import (  # noqa: E402
    aggregate_seed_rows,
    run_seed_probe,
)


class StageBMidiToSoloControlledScaleCheckpointDeadAirRepeatabilityTemperatureGuardRepairProbeError(
    ValueError
):
    pass
...
PASS_NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_repair_consolidation"
)
FAIL_NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_followup_decision"
)
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_"
    "temperature_guard_repair_probe_v1"
)
...
def validate_guard_decision(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(report.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointDeadAirRepeatabilityTemperatureGuardRepairProbeError(
    ...
    *,
    ...
    return {
        ...
        "readiness": {
            ...
            "temperature_guard_repair_target_qualified": bool(target_qualified),
        ...
        "decision": {
            ...
            "reason": (
                ...
                "removes the repeatability strict shortfall"
            ),
        },
        "not_proven": [
        ...
        "next_recommended_issue": (
            "Stage B MIDI-to-solo controlled checkpoint dead-air temperature guard repair consolidation"
            if target_qualified
...
def validate_repair_report(
    ...
    *,
    ...
    return {
        ...
        "temperature_guard_repair_target_qualified": bool(
            readiness.get("temperature_guard_repair_target_qualified", False)
        ),
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- temperature guard repair target qualified: `{_bool_token(readiness['temperature_guard_repair_target_qualified'])}`",
        ...
        f"- dead-air failure count: `{failure['dead_air_failure_count']}`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run controlled dead-air repeatability temperature guard repair probe"
    )
    parser.add_argument(
        ...
        "harness_stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_decision/"
    ...
    parser.add_argument("--issue_number", type=int, default=568)
...
def main() -> int:
    ...
    write_json(
        run_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_repair_probe_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        run_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_repair_probe.md",
        markdown,
    )
    if args.doc_path:
