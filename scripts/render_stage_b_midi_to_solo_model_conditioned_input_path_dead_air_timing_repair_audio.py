...
class StageBMidiToSoloModelConditionedInputPathDeadAirTimingRepairAudioError(
    ValueError
):
    pass
...
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_model_conditioned_input_path_"
    "dead_air_timing_repair_audio_package_v1"
)
...
def _require_no_quality_claim(container: dict[str, Any], *, label: str) -> None:
    ...
    if claimed:
        raise StageBMidiToSoloModelConditionedInputPathDeadAirTimingRepairAudioError(
...
def validate_source_report(report: dict[str, Any], *, expected_count: int) -> list[dict[str, Any]]:
    ...
    required_true = [
        "dead_air_timing_repair_probe_completed",
        "repaired_ranked_midi_written",
        "dead_air_timing_repair_passed",
        "dead_air_timing_audio_render_required",
    ]
    missing = [name for name in required_true if not bool(readiness.get(name, False))]
    if missing:
        raise StageBMidiToSoloModelConditionedInputPathDeadAirTimingRepairAudioError(
            f"missing repair probe readiness: {missing}"
        )
    ...
    for row in rows[: int(expected_count)]:
        ...
        if not bool(row.get("candidate_repair_passed", False)):
            raise StageBMidiToSoloModelConditionedInputPathDeadAirTimingRepairAudioError(
                "candidate repair should pass before audio render"
            )
        repaired_metrics = _dict(row.get("repaired_metrics"))
        repair_stats = _dict(row.get("repair_stats"))
        compacted.append(
            {
                ...
                "source_dead_air_ratio": _float(
                    _dict(row.get("source_metrics")).get("dead_air_ratio")
                ),
                ...
                "added_note_ratio": _float(repair_stats.get("added_note_ratio")),
                "postprocess_removal_ratio": _float(
                    repair_stats.get("postprocess_removal_ratio")
                ),
            }
        )
    return compacted
        ...
        rendered.append(
            {
                ...
                "added_note_ratio": item["added_note_ratio"],
...
def build_audio_render_report(
    ...
    *,
    ...
    added_ratios = [_float(item.get("added_note_ratio")) for item in candidates]
    removal_ratios = [_float(item.get("postprocess_removal_ratio")) for item in candidates]
    return {
        ...
        "summary": {
            ...
            "max_added_note_ratio": max(added_ratios, default=0.0),
            "max_postprocess_removal_ratio": max(removal_ratios, default=0.0),
            "max_repaired_interval": max(repaired_interval_values, default=0),
            "remaining_wide_interval_risk": max(repaired_interval_values, default=0) >= 12,
        },
        ...
        "decision": {
            ...
            "reason": (
                "repaired dead-air/timing MIDI rendered to WAV; next boundary should review objective remaining risks without claiming quality"
            ),
        },
        "not_proven": [
...
def validate_audio_render_report(
    ...
    *,
    ...
    if require_audio_package_completed and not bool(
        boundary.get("dead_air_timing_repair_audio_package_completed", False)
    ):
        raise StageBMidiToSoloModelConditionedInputPathDeadAirTimingRepairAudioError(
    ...
    return {
        ...
        "dead_air_timing_repair_audio_package_completed": bool(
            boundary.get("dead_air_timing_repair_audio_package_completed", False)
        ),
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- remaining wide-interval risk: `{_bool_token(summary['remaining_wide_interval_risk'])}`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Render model-conditioned dead-air timing repair WAV files"
    )
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_input_path_dead_air_timing_repair_audio_package.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_input_path_dead_air_timing_repair_audio_package_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_input_path_dead_air_timing_repair_audio_package.md",
        markdown,
    )
    if args.doc_path:
