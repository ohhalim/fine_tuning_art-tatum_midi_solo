"""Render density-aftercare repaired solo-yield MIDI candidates to WAV."""
...
from scripts.run_music_transformer_solo_yield_density_aftercare_sweep import (  # noqa: E402
    SCHEMA_VERSION as DENSITY_REPAIR_SCHEMA_VERSION,
    build_repair_sweep,
    read_json,
)


SCHEMA_VERSION = "music_transformer_solo_yield_density_aftercare_audio_package_v1"
BOUNDARY = "music_transformer_solo_yield_density_aftercare_audio_package"
NEXT_BOUNDARY = "music_transformer_solo_yield_density_aftercare_listening_package"
CommandRunner = Callable[[Sequence[str]], subprocess.CompletedProcess[str]]


class SoloYieldDensityAftercareAudioPackageError(ValueError):
    pass
...
def _require_no_quality_claim(report: dict[str, Any]) -> None:
    ...
    if claimed:
        raise SoloYieldDensityAftercareAudioPackageError(f"unexpected quality claim: {claimed}")


def _validate_repair_sweep(report: dict[str, Any]) -> list[dict[str, Any]]:
    if str(report.get("schema_version") or "") != DENSITY_REPAIR_SCHEMA_VERSION:
        raise SoloYieldDensityAftercareAudioPackageError("density aftercare repair schema required")
    _require_no_quality_claim(report)
    aggregate = _dict(report.get("aggregate"))
    if not bool(aggregate.get("target_supported", False)):
        raise SoloYieldDensityAftercareAudioPackageError("target support required")
    rows = [_dict(item) for item in _list(report.get("candidate_repairs"))]
    if not rows:
        raise SoloYieldDensityAftercareAudioPackageError("candidate repairs required")
    for row in rows:
        midi_path = Path(str(row.get("repaired_midi_path") or ""))
        if not midi_path.exists():
            raise SoloYieldDensityAftercareAudioPackageError(f"repaired MIDI missing: {midi_path}")
    return rows
...
def load_or_build_repair_sweep(
    *,
    ...
    return build_repair_sweep(
        ...
        output_dir=output_dir / "source_density_aftercare",
    )
...
def build_render_plan(
    ...
    *,
    ...
    if not renderer.exists():
        raise SoloYieldDensityAftercareAudioPackageError(f"renderer not found: {renderer}")
    if not soundfont.exists():
        raise SoloYieldDensityAftercareAudioPackageError(f"soundfont not found: {soundfont}")
    ...
    for row in repairs:
        ...
        wav_path = output_dir / "audio" / f"candidate_{review_index:02d}_{case_label}_density_aftercare.wav"
        plan.append(
            {
...
def execute_render_plan(plan: list[dict[str, Any]], *, runner: CommandRunner = default_runner) -> list[dict[str, Any]]:
    ...
    for item in plan:
        ...
        if completed.returncode != 0:
            raise SoloYieldDensityAftercareAudioPackageError(
    ...
    *,
    ...
    report = {
        ...
        "source_repair_sweep": {
            ...
            "low_note_count_before": _int(source_aggregate.get("low_note_count_before")),
            "low_note_count_after": _int(source_aggregate.get("low_note_count_after")),
            "inserted_note_total": _int(source_aggregate.get("inserted_note_total")),
        ...
        "decision": {
            ...
            "selected_next_target": "density_aftercare_listening_package",
            "next_boundary": NEXT_BOUNDARY,
            "critical_user_input_required": False,
            "reason": "density-aftercare repaired MIDI rendered to WAV for technical listening package",
        },
        "not_proven": [
    ...
    write_json(output_dir / "density_aftercare_audio_package.json", report)
    write_json(output_dir / "density_aftercare_audio_package_summary.json", validate_report(report))
    write_text(output_dir / "density_aftercare_audio_package.md", markdown_report(report))
    return report


def validate_report(report: dict[str, Any], *, min_wav_count: int = 1) -> dict[str, Any]:
    if str(report.get("schema_version") or "") != SCHEMA_VERSION:
        raise SoloYieldDensityAftercareAudioPackageError("schema version mismatch")
    readiness = _dict(report.get("readiness"))
    aggregate = _dict(report.get("aggregate"))
    rendered_count = _int(aggregate.get("rendered_wav_count"))
    if rendered_count < int(min_wav_count):
        raise SoloYieldDensityAftercareAudioPackageError("rendered WAV count below requirement")
    if not bool(readiness.get("audio_package_completed", False)):
        raise SoloYieldDensityAftercareAudioPackageError("audio package completion required")
    if not bool(aggregate.get("technical_wav_validation", False)):
        raise SoloYieldDensityAftercareAudioPackageError("technical WAV validation required")
    _require_no_quality_claim(report)
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- source low note count: `{source['low_note_count_before']} -> {source['low_note_count_after']}`",
        f"- source inserted note total: `{source['inserted_note_total']}`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render density aftercare repaired MIDI to WAV")
    parser.add_argument(
        "--repair_sweep_report",
        type=str,
        default=(
    ...
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/music_transformer_finetune_mvp/solo_yield_density_aftercare_audio",
    )
...
def main() -> int:
    ...
    write_json(output_dir / "density_aftercare_audio_package_summary.json", summary)
    if args.doc_path:
