...
from scripts.render_stage_b_generic_tiny_checkpoint_repair_audio import (  # noqa: E402
    sha256_file,
    wav_meta,
)
...
class StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(ValueError):
    pass
...
def required_review_items(local_audio_render_package: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if str(boundary.get("boundary") or "") != (
        ...
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
            "unexpected phrase-continuation audio render package boundary"
        )
    if str(boundary.get("status") or "") != "ready_for_local_render":
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
            "audio render package is not ready for local render"
        )
    if bool(boundary.get("render_attempted", True)):
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
            "source package must not already attempt rendering"
        )
    if bool(boundary.get("audio_rendered_quality_claimed", True)):
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
            "source package must not claim audio quality"
        )
    if bool(boundary.get("human_audio_preference_claimed", True)):
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
            "source package must not claim human preference"
        )
    review_items = _list(local_audio_render_package.get("review_items"))
    if not review_items:
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError("review items required")
    ...
    for item in review_items:
        ...
        if bool(midi_file.get("required", False)) and not Path(midi_path).exists():
            raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
                f"required MIDI not found: {midi_path}"
            )
        if not bool(item.get("target_qualified", False)):
            raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
                "review item must remain target-qualified"
            )
        compacted.append(
            {
    ...
    if not compacted:
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError("no renderable review items")
    return compacted
...
def build_render_plan(
    ...
    *,
    ...
    if not renderer.exists():
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(f"renderer not found: {renderer}")
    if not soundfont.exists():
        raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(f"soundfont not found: {soundfont}")
...
def build_audio_render_report(
    ...
    *,
    ...
    return {
        ...
        "decision": {
            "current_boundary": "stage_b_generic_tiny_checkpoint_repair_phrase_continuation_local_audio_render_attempt",
            "next_boundary": "stage_b_generic_tiny_checkpoint_repair_phrase_continuation_user_listening_review_input",
        ...
        "next_recommended_issue": "Stage B generic tiny checkpoint repair phrase continuation user listening review input",
    }
    ...
    *,
    ...
    for item in files:
        ...
        if not bool(wav_file.get("exists", False)):
            raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
                f"missing rendered wav for rank {item.get('review_rank')}"
            )
        ...
        if int(wav_file.get("frame_count", 0) or 0) <= 0:
            raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
                f"empty wav for rank {item.get('review_rank')}"
            )
        if int(wav_file.get("size_bytes", 0) or 0) <= 44:
            raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
                f"invalid wav size for rank {item.get('review_rank')}"
            )
    if require_no_quality_claim:
        ...
        if any(claimed):
            raise StageBGenericTinyCheckpointRepairPhraseContinuationAudioRenderError(
                "quality or preference claims must not be set"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Render phrase-continuation repair audio")
...
def main() -> int:
    ...
    write_json(
        output_dir / "stage_b_generic_tiny_checkpoint_repair_phrase_continuation_local_audio_render_attempt.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_generic_tiny_checkpoint_repair_phrase_continuation_local_audio_render_attempt_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(output_dir / "stage_b_generic_tiny_checkpoint_repair_phrase_continuation_local_audio_render_attempt.md", markdown)
    if args.doc_path:
