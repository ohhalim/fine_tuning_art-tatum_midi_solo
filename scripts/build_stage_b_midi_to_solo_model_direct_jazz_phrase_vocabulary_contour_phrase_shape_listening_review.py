"""Build pending listening review boundary for contour phrase-shape WAV candidates."""
...
from scripts.build_stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_audio_package import (  # noqa: E402
...
class StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeListeningReviewError(ValueError):
    pass


BOUNDARY = "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_listening_review"
NEXT_BOUNDARY = "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_objective_only_next_decision"
SCHEMA_VERSION = "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_listening_review_v1"


def read_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeListeningReviewError(
...
def compact_rendered_candidate(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "small_interval_ratio_le4": _float(item.get("small_interval_ratio_le4")),
...
def validate_audio_package(audio_package: dict[str, Any], expected_count: int) -> list[dict[str, Any]]:
    ...
    if str(boundary.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeListeningReviewError(
            "contour phrase-shape audio package boundary required"
        )
    if str(decision.get("next_boundary") or "") != BOUNDARY:
        raise StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeListeningReviewError(
            "audio package must route to contour phrase-shape listening review"
        )
...
def listening_review_template(candidates: list[dict[str, Any]]) -> str:
    lines = [
        "# Stage B MIDI-to-Solo Contour Phrase-Shape Listening Review Input",
        "",
        "## Review Status",
        "",
        "- reviewer: `pending`",
        ...
        "## Candidates",
        "",
        "| rank | midi | wav | note count | max interval | small interval ratio | flags | decision |",
        "|---:|---|---|---:|---:|---:|---|---|",
    ]
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    ",".join(str(flag) for flag in item.get("analysis_flags", [])) or "none",
                    "`pending`",
                ]
            )
            + " |"
...
def build_listening_review_report(
    ...
    *,
    ...
    review_input_path = output_dir / "review" / "contour_phrase_shape_listening_review_input.md"
    ...
    return {
        ...
        "next_recommended_issue": "Stage B MIDI-to-solo model-direct jazz phrase vocabulary contour phrase-shape objective-only next decision",
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build contour phrase-shape listening review boundary")
    parser.add_argument("--audio_package", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_listening_review",
    )
...
def main() -> int:
    ...
    write_json(output_dir / "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_listening_review.json", report)
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_listening_review_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(output_dir / "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_listening_review.md", markdown)
    if args.doc_path:
