"""Decide the objective-only next boundary after controlled postprocess removal dead-air repair review."""
...
from scripts.build_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_listening_review import (  # noqa: E402
    BOUNDARY as SOURCE_BOUNDARY,
)
from scripts.consolidate_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair import (  # noqa: E402
    BOUNDARY as CONSOLIDATION_BOUNDARY,
)


class StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextError(
    ValueError
):
    pass
...
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "postprocess_removal_dead_air_repair_objective_next_v1"
)
...
def validate_listening_review_boundary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(boundary.get("boundary") or report.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextError(
            "postprocess removal dead-air repair listening review boundary required"
        )
...
def validate_postprocess_removal_dead_air_repair_consolidation(report: dict[str, Any]) -> dict[str, Any]:
    if str(report.get("boundary") or "") != CONSOLIDATION_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextError(
            "postprocess removal dead-air repair consolidation boundary required"
        )
    evidence = _dict(report.get("evidence_summary"))
    result = _dict(report.get("consolidation_result"))
    readiness = _dict(report.get("readiness"))
    if not bool(result.get("objective_midi_support", False)):
        raise StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextError(
            "objective MIDI support required"
        )
    ...
    target_avg_postprocess_removal = _float(evidence.get("target_avg_postprocess_removal_ratio"))
    if target_avg_postprocess_removal and _float(evidence.get("avg_postprocess_removal_ratio")) > target_avg_postprocess_removal:
        raise StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextError(
            "average postprocess removal ratio should stay within target"
        )
    blocked_claims = [
        "human_audio_preference_claimed",
        "midi_to_solo_musical_quality_claimed",
    ...
    return {
        ...
        "source_strict_valid_sample_count": _int(evidence.get("source_strict_valid_sample_count")),
        "source_dead_air_failure_count": _int(evidence.get("source_dead_air_failure_count")),
        "source_avg_postprocess_removal_ratio": _float(
            evidence.get("source_avg_postprocess_removal_ratio")
        ),
        ...
        "max_postprocess_removal_ratio": _float(evidence.get("max_postprocess_removal_ratio")),
        "target_avg_postprocess_removal_ratio": target_avg_postprocess_removal,
...
def build_objective_next_decision_report(
    ...
    *,
    ...
    evidence = validate_postprocess_removal_dead_air_repair_consolidation(consolidation)
    objective_path_supported = (
        bool(evidence.get("objective_midi_support", False))
        ...
        and _float(evidence.get("avg_postprocess_removal_ratio"))
        <= _float(evidence.get("target_avg_postprocess_removal_ratio"))
        and not bool(review.get("validated_review_input_present", True))
    )
    if not objective_path_supported:
        raise StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextError(
            "objective postprocess removal dead-air repair path support required"
        )
    boundary = {
        "boundary": BOUNDARY,
        ...
        "objective_postprocess_removal_dead_air_repair_path_supported": True,
    ...
    return {
        ...
        "postprocess_removal_dead_air_repair_summary": evidence,
        ...
        "decision": {
            ...
            "reason": (
                "postprocess removal dead-air repair objective MIDI evidence is supported; listening review input remains pending, "
                "so close this objective path and route the next boundary to MVP evidence consolidation"
            ),
        },
        "proven": [
            "postprocess_removal_dead_air_repair_strict_valid_9_of_9",
...
def validate_objective_next_decision_report(
    ...
    *,
    ...
    evidence = _dict(report.get("postprocess_removal_dead_air_repair_summary"))
    ...
    if require_objective_support and not bool(
        readiness.get("objective_postprocess_removal_dead_air_repair_path_supported", False)
    ):
        raise StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextError(
            "objective postprocess removal dead-air repair path support required"
        )
    ...
    return {
        ...
        "objective_postprocess_removal_dead_air_repair_path_supported": bool(
            readiness.get("objective_postprocess_removal_dead_air_repair_path_supported", False)
        ),
        "validated_review_input_present": bool(readiness.get("validated_review_input_present", True)),
        "preference_fill_allowed": bool(readiness.get("preference_fill_allowed", True)),
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    evidence = report["postprocess_removal_dead_air_repair_summary"]
    lines = [
        ...
        f"- objective postprocess removal dead-air repair path supported: `{_bool_token(boundary['objective_postprocess_removal_dead_air_repair_path_supported'])}`",
        ...
        f"- temperature: `{evidence['temperature']:.2f}`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Decide controlled postprocess removal dead-air repair objective-only next boundary")
    parser.add_argument(
        "--listening_review",
        type=str,
        default=(
            "outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
            "postprocess_removal_dead_air_repair_listening_review/harness_stage_b_midi_to_solo_"
            "controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_listening_review/"
            "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
            "postprocess_removal_dead_air_repair_listening_review.json"
        ),
    )
    parser.add_argument(
        ...
        default=(
            "outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
            "postprocess_removal_dead_air_repair_consolidation/harness_stage_b_midi_to_solo_"
            "controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_consolidation/"
            "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
            "postprocess_removal_dead_air_repair_consolidation.json"
        ),
    )
    parser.add_argument(
        "--output_root",
        ...
        default=(
            "outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
            "postprocess_removal_dead_air_repair_objective_next"
        ),
    )
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_objective_next.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_objective_next_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_objective_next.md",
        markdown,
    )
    if args.doc_path:
