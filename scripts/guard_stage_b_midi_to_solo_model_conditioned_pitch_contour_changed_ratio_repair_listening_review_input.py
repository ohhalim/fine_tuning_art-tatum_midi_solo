"""Guard changed-ratio repair listening review preference fill against missing input."""
...
from scripts.build_stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_listening_review_package import (  # noqa: E402
    BOUNDARY as SOURCE_BOUNDARY,
    NEXT_BOUNDARY as SOURCE_NEXT_BOUNDARY,
)


class StageBMidiToSoloPitchContourChangedRatioRepairListeningInputGuardError(
    ValueError
):
    pass
...
FILL_BOUNDARY = "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_listening_review_fill"
OBJECTIVE_NEXT_BOUNDARY = "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_objective_only_next_decision"
SCHEMA_VERSION = "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_listening_review_input_guard_v1"
...
def _require_no_quality_claim(container: dict[str, Any], *, label: str) -> None:
    ...
    if claimed:
        raise StageBMidiToSoloPitchContourChangedRatioRepairListeningInputGuardError(
...
def validate_source_package(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if boundary != SOURCE_BOUNDARY:
        raise StageBMidiToSoloPitchContourChangedRatioRepairListeningInputGuardError(
            "changed-ratio repair listening review package boundary required"
        )
    if str(decision.get("next_boundary") or "") != SOURCE_NEXT_BOUNDARY:
        raise StageBMidiToSoloPitchContourChangedRatioRepairListeningInputGuardError(
            "changed-ratio repair listening package must route to input guard"
        )
    ...
    _require_no_quality_claim(readiness, label="changed-ratio repair listening package readiness")
    return {
    ...
    *,
    ...
    return {
        ...
        "next_recommended_issue": (
            "Stage B MIDI-to-solo model-conditioned pitch-contour changed-ratio repair listening review fill"
            if validated_input
            else "Stage B MIDI-to-solo model-conditioned pitch-contour changed-ratio repair objective-only next decision"
        ),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Guard changed-ratio repair listening review input"
    )
    parser.add_argument("--source_package", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_listening_review_input_guard",
    )
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--doc_path", type=str, default="")
    parser.add_argument("--issue_number", type=int, default=724)
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_listening_review_input_guard.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_listening_review_input_guard_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_listening_review_input_guard.md",
        markdown,
    )
    if args.doc_path:
