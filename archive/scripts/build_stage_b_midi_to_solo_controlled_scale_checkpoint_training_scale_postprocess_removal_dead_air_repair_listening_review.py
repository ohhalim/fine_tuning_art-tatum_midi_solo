"""Build pending listening review boundary for controlled postprocess-removal-dead-air-repair WAVs."""
...
from scripts.build_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_audio_package import (  # noqa: E402
...
class StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairListeningReviewError(
    ValueError
):
    pass
...
BOUNDARY = (
    ...
    "postprocess_removal_dead_air_repair_listening_review"
)
NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
...
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "postprocess_removal_dead_air_repair_listening_review_v1"
)
...
def compact_rendered_candidate(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if not bool(wav_file.get("exists", False)) or not wav_path.exists():
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairListeningReviewError(
    ...
    return {
        ...
        "postprocess_removal_ratio": _float(item.get("postprocess_removal_ratio")),
    }
...
def validate_audio_package(audio_package: dict[str, Any], expected_count: int) -> list[dict[str, Any]]:
    ...
    if str(boundary.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairListeningReviewError(
            "postprocess removal dead-air repair audio package boundary required"
        )
    if str(decision.get("next_boundary") or "") != BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairListeningReviewError(
            "audio package must route to listening review"
        )
    ...
    claimed = [name for name in blocked if bool(boundary.get(name, False))]
    if claimed:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairListeningReviewError(
...
def listening_review_template(candidates: list[dict[str, Any]]) -> str:
    lines = [
        "# Stage B MIDI-to-Solo Controlled Postprocess Removal Dead-Air Repair Listening Review Input",
        "",
        "## Review Status",
        "",
        "- reviewer: `pending`",
...
def build_listening_review_report(
    ...
    *,
    ...
    review_input_path = output_dir / "review" / "postprocess_removal_dead_air_repair_listening_review_input.md"
    ...
    return {
        ...
        "decision": {
            ...
            "reason": (
                "postprocess removal dead-air repair listening review input remains pending; route to objective-only "
                "next decision without preference claim"
            ),
        },
        "not_proven": [
            "listening_review_completed",
...
def validate_listening_review_report(
    ...
    *,
    ...
    summary = _dict(report.get("review_input_summary"))
    ...
    return {
        ...
        "pending_status_field_count": len(_list(summary.get("pending_status_fields"))),
        "pending_candidate_decision_count": len(_list(summary.get("pending_candidate_decisions"))),
        "pending_candidate_field_count": len(_list(summary.get("pending_candidate_fields"))),
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Stage B MIDI-to-Solo Controlled Postprocess Removal Dead-Air Repair Listening Review",
        "",
        "## Summary",
        "",
        ...
        "## Pending Fields",
        "",
        f"- pending status field count: `{len(_list(summary.get('pending_status_fields')))}`",
        f"- pending candidate decision count: `{len(_list(summary.get('pending_candidate_decisions')))}`",
        f"- pending candidate field count: `{len(_list(summary.get('pending_candidate_fields')))}`",
        "",
        "## Review Input",
        "",
        f"- template path: `{report['review_input_template_path']}`",
        "",
        "## Candidates",
        "",
    ]
    for item in report["review_candidates"]:
        lines.append(
            "- rank `{rank}` seed `{seed}` sample `{sample}` wav `{wav}`".format(
                rank=item["rank"],
                seed=item["seed"],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build controlled postprocess removal dead-air repair listening review boundary"
    )
    parser.add_argument("--audio_package", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_listening_review",
    )
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_listening_review.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_listening_review_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_listening_review.md",
        markdown,
    )
    if args.doc_path:
