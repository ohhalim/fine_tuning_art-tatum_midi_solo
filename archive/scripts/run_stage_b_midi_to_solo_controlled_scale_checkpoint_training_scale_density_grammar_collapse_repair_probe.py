"""Run density/grammar/collapse repair probe for the selected-scale checkpoint."""
...
from scripts.decide_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_repair import (  # noqa: E402
...
class StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError(ValueError):
    pass
...
FAIL_NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "density_grammar_collapse_remaining_blocker_decision"
)
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "density_grammar_collapse_repair_probe_v1"
)
...
def validate_decision(report: dict[str, Any]) -> dict[str, Any]:
    if str(report.get("boundary") or "") != DECISION_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("repair decision boundary required")
    ...
    if str(decision.get("next_boundary") or "") != BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("decision must route to repair probe")
    if str(decision.get("selected_target") or "") != "target_density_grammar_collapse_postprocess_repair":
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("density/grammar/collapse repair target required")
    if not bool(evidence.get("collapse_across_all_samples", False)):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("collapse evidence required")
    if not bool(evidence.get("partial_grammar_failure", False)):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("partial grammar failure evidence required")
    blocked = [
    ...
    if claimed:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError(f"unexpected quality claim: {claimed}")
    return {
...
def validate_baseline(report: dict[str, Any]) -> dict[str, Any]:
    if str(report.get("boundary") or "") != BASELINE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("baseline generation boundary required")
    generation = _dict(report.get("generation_summary"))
    readiness = _dict(report.get("readiness"))
    if not bool(readiness.get("generation_path_executable", False)):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("baseline generation path required")
    if _int(generation.get("sample_count")) <= 0:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("baseline samples required")
    if _int(generation.get("note_count_failure_count")) <= 0 and not _dict(generation.get("diagnostic_failure_reasons")):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("baseline failure evidence required")
    ...
    grammar_failures = _count_prefixed(strict, "grammar_gate_failed") or max(
        0, sample_count - grammar_gate_sample_count
    )
    return {
...
def _repair_metrics(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "grammar_failure_count": _count_prefixed(_dict(summary.get("strict_failure_reasons")), "grammar_gate_failed"),
...
def build_repair_report(
    *,
    ...
    grammar_failure_delta = baseline_summary["grammar_failure_count"] - repair_summary["grammar_failure_count"]
    ...
    return {
        ...
        "comparison": {
            ...
            "grammar_failure_delta": int(grammar_failure_delta),
            ...
            "density_grammar_collapse_target_supported": bool(target_supported),
            "strict_gate_recovered": bool(strict_recovered),
        },
        "readiness": {
            "boundary": BOUNDARY,
            "density_grammar_collapse_repair_probe_completed": _int(generation_result.get("returncode")) == 0,
            "density_grammar_collapse_target_supported": bool(target_supported),
            "strict_gate_recovered": bool(strict_recovered),
        ...
        "decision": {
            ...
            "reason": (
                "density/grammar/collapse target support is measured; repeatability is required "
                "when the strict gate is recovered for only a small sample set"
            ),
        },
        "not_proven": [
        ...
        "next_recommended_issue": (
            ...
            if strict_recovered
            else "Stage B MIDI-to-solo controlled scale checkpoint training scale density grammar collapse remaining blocker decision"
        ),
    }
...
def validate_repair_report(
    ...
    *,
    ...
    if expected_boundary and boundary != expected_boundary:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("unexpected boundary")
    if expected_next_boundary and str(decision.get("next_boundary") or "") != expected_next_boundary:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("unexpected next boundary")
    if _int(command.get("returncode")) != 0:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("generation command must succeed")
    if require_target_supported and not bool(comparison.get("density_grammar_collapse_target_supported", False)):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("repair target support required")
    ...
    return {
        ...
        "grammar_failure_delta": _int(comparison.get("grammar_failure_delta")),
        ...
        "density_grammar_collapse_target_supported": bool(comparison.get("density_grammar_collapse_target_supported", False)),
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- density/grammar/collapse target supported: `{_bool_token(comparison['density_grammar_collapse_target_supported'])}`",
        ...
        f"- note-count / grammar / dead-air failure count: `{repair['note_count_failure_count']}` / `{repair['grammar_failure_count']}` / `{repair['dead_air_failure_count']}`",
        ...
        f"- grammar failure delta: `{comparison['grammar_failure_delta']}`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run selected-scale checkpoint density/grammar/collapse repair probe")
    parser.add_argument(
        "--decision_report",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_repair_decision/"
        "harness_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_repair_decision/"
    ...
    parser.add_argument("--output_root", type=str, default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repair_probe")
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--doc_path", type=str, default="")
    parser.add_argument("--issue_number", type=int, default=586)
...
def main() -> int:
    ...
    if not (checkpoint_dir / "checkpoint_epoch1.pt").exists():
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepairProbeError("checkpoint_epoch1.pt required")

    probe_output_root = run_dir / "generation_probe"
    probe_run_id = "density_grammar_collapse_repair"
    ...
    write_json(run_dir / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repair_probe.json", report)
    write_json(
        run_dir / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repair_probe_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(run_dir / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repair_probe.md", markdown)
    if args.doc_path:
