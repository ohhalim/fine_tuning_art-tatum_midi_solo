"""Guard repaired phrase-bank listening preference fill against missing review input."""
...
from scripts.build_stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_package import (  # noqa: E402
    BOUNDARY as SOURCE_BOUNDARY,
    NEXT_BOUNDARY as SOURCE_NEXT_BOUNDARY,
)


class StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningInputGuardError(ValueError):
    pass


BOUNDARY = "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_input_guard"
FILL_BOUNDARY = "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_fill"
OBJECTIVE_NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_objective_only_next_decision"
)
SCHEMA_VERSION = "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_input_guard_v1"
...
def _require_no_quality_claim(container: dict[str, Any], *, label: str) -> None:
    ...
    if claimed:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningInputGuardError(
...
def validate_source_package(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(report.get("boundary") or readiness.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningInputGuardError(
            "repaired listening review package boundary required"
        )
    if str(decision.get("next_boundary") or "") != SOURCE_NEXT_BOUNDARY:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningInputGuardError(
            "review package must route to repaired input guard"
        )
    ...
    if _int(readiness.get("review_item_count")) <= 0:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningInputGuardError("review items required")
    ...
    *,
    ...
    return {
        ...
        "next_recommended_issue": (
            "Stage B MIDI-to-solo phrase-bank dead-air density repair listening review fill"
            if validated_input
            else "Stage B MIDI-to-solo phrase-bank dead-air density repair objective-only next decision"
        ),
    }
...
def validate_listening_review_input_guard_report(
    ...
    *,
    ...
    if expected_next_boundary and str(decision.get("next_boundary") or "") != expected_next_boundary:
        raise StageBMidiToSoloPhraseBankDeadAirDensityRepairListeningInputGuardError("unexpected next boundary")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Guard repaired phrase-bank listening review input")
    parser.add_argument("--source_package", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_input_guard",
    )
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--doc_path", type=str, default="")
    parser.add_argument("--issue_number", type=int, default=648)
...
def main() -> int:
    ...
    write_json(
        output_dir / "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_input_guard.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_input_guard_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir / "stage_b_midi_to_solo_phrase_bank_dead_air_density_repair_listening_review_input_guard.md",
        markdown,
    )
    if args.doc_path:
