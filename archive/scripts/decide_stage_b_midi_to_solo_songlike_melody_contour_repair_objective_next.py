...
from scripts.guard_stage_b_midi_to_solo_songlike_melody_contour_repair_listening_review_input import (  # noqa: E402
    ...
    EXPECTED_SOURCE_SCHEMA_VERSIONS as SOURCE_GUARD_SCHEMA_VERSIONS,
...
SCHEMA_VERSION = "stage_b_midi_to_solo_songlike_melody_contour_repair_objective_next_v5"

EXPECTED_SOURCE_SCHEMA_VERSIONS = {
    "songlike_melody_contour_repair_listening_review_input_guard": SOURCE_GUARD_SCHEMA_VERSION,
    **SOURCE_GUARD_SCHEMA_VERSIONS,
}
...
def _source_context_fields(source: dict[str, Any]) -> dict[str, Any]:
    ...
    missing_objective_preserved = [
        ...
        if not bool(source.get(f"objective_{key}"))
    ]
...
def validate_input_guard_report(report: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        source_validation_summary = validate_listening_review_input_guard_report(
            report,
    ...
    except StageBMidiToSoloSonglikeMelodyContourRepairListeningInputGuardError as exc:
        raise StageBMidiToSoloSonglikeMelodyContourRepairObjectiveNextError(
            str(exc)
        ) from exc
    ...
    if _int(source.get("objective_source_outside_soloing_repair_wav_count")) < 6:
        raise StageBMidiToSoloSonglikeMelodyContourRepairObjectiveNextError(
            "objective source outside-soloing WAV count below 6"
        )
    ...
    return {
        ...
        **source_context,
        ...
        "source_candidate_failure_labeling_schema_version": str(
            source_validation_summary.get("source_candidate_failure_labeling_schema_version") or ""
        ),
    ...
    *,
    ...
    return {
        ...
        "objective_summary": {
            ...
            "source_songlike_melody_contour_repair_audio_package_schema_version": str(
                source["source_songlike_melody_contour_repair_audio_package_schema_version"]
            ),
            "source_songlike_melody_contour_repair_sweep_schema_version": str(
                source["source_songlike_melody_contour_repair_sweep_schema_version"]
            ),
            "source_targeted_quality_repair_followup_schema_version": str(
                source["source_targeted_quality_repair_followup_schema_version"]
            ),
            "source_targeted_quality_repair_objective_next_schema_version": str(
                source["source_targeted_quality_repair_objective_next_schema_version"]
            ),
            "source_targeted_quality_repair_sweep_schema_version": str(
            ...
            "objective_source_outside_soloing_repair_schema_context_preserved": bool(
                source["objective_source_outside_soloing_repair_schema_context_preserved"]
            ),
            "objective_source_outside_soloing_repair_objective_schema_version": str(
                source["objective_source_outside_soloing_repair_objective_schema_version"]
            ),
            "source_outside_soloing_repair_schema_context_preserved": bool(
...
def validate_objective_next_report(
    ...
    *,
    ...
    if str(report.get("schema_version") or "") != SCHEMA_VERSION:
        raise StageBMidiToSoloSonglikeMelodyContourRepairObjectiveNextError(
            "songlike melody contour repair objective next schema version mismatch"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--issue_number", type=int, default=1192)
