"""Run repeatability probe for selected-scale density/grammar/collapse repair."""
...
class StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepeatabilityProbeError(
    ValueError
):
    pass
...
PASS_NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "density_grammar_collapse_repeatability_consolidation"
)
...
FAIL_NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "density_grammar_collapse_followup_decision"
)
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "density_grammar_collapse_repeatability_probe_v1"
)
...
def parse_seeds(raw: str) -> list[int]:
    ...
    if not seeds:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepeatabilityProbeError(
            "seed list required"
        )
    return seeds
...
def validate_repair_probe(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(report.get("boundary") or readiness.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepeatabilityProbeError(
            "density/grammar/collapse repair boundary required"
        )
    if str(decision.get("next_boundary") or "") != BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepeatabilityProbeError(
            "density/grammar/collapse repair must route to repeatability probe"
        )
    if not bool(readiness.get("density_grammar_collapse_target_supported", False)):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepeatabilityProbeError(
            "density/grammar/collapse repair target support required"
        )
    ...
    return {
        ...
        "source_note_count_failure_count": _int(repair.get("note_count_failure_count")),
        "source_grammar_failure_count": _int(repair.get("grammar_failure_count")),
        "source_dead_air_failure_count": _int(repair.get("dead_air_failure_count")),
        "source_collapse_warning_sample_count": _int(repair.get("collapse_warning_sample_count")),
...
def run_seed_probe(
    ...
    *,
    ...
    return {
        ...
        "note_count_failure_count": _count_prefixed(
            _dict(summary.get("diagnostic_failure_reasons")), "note count too low:"
        ),
        "grammar_failure_count": _count_prefixed(
            _dict(summary.get("strict_failure_reasons")), "grammar_gate_failed"
        ),
        "dead_air_failure_count": _count_prefixed(
            _dict(summary.get("diagnostic_failure_reasons")), "dead-air ratio too high:"
        ),
...
def aggregate_seed_rows(seed_rows: Sequence[dict[str, Any]]) -> dict[str, Any]:
    ...
    total_note_count_failure = sum(_int(row.get("note_count_failure_count")) for row in seed_rows)
    total_grammar_failure = sum(_int(row.get("grammar_failure_count")) for row in seed_rows)
    total_dead_air_failure = sum(_int(row.get("dead_air_failure_count")) for row in seed_rows)
    ...
    return {
        ...
        "note_count_failure_count": int(total_note_count_failure),
        "grammar_failure_count": int(total_grammar_failure),
        "dead_air_failure_count": int(total_dead_air_failure),
...
def build_repeatability_report(
    *,
    ...
    target_qualified = (
        ...
        and _float(aggregate["avg_postprocess_removal_ratio"]) <= 0.49
    )
    strict_gate_stable = bool(aggregate["all_samples_strict_valid"])
    dead_air_remaining = _int(aggregate["dead_air_failure_count"]) > 0
    if target_qualified and strict_gate_stable:
        next_boundary = PASS_NEXT_BOUNDARY
    elif target_qualified and dead_air_remaining:
        next_boundary = DEAD_AIR_NEXT_BOUNDARY
    else:
    ...
    return {
        ...
        "comparison": {
            ...
            "strict_gate_stable": bool(strict_gate_stable),
            "dead_air_remaining": bool(dead_air_remaining),
        },
        "readiness": {
            "boundary": BOUNDARY,
            "density_grammar_collapse_repeatability_probe_completed": True,
            "density_grammar_collapse_repeatability_target_supported": bool(target_qualified),
            "strict_gate_stable": bool(strict_gate_stable),
            "dead_air_remaining": bool(dead_air_remaining),
        ...
        "decision": {
            ...
            "reason": (
                "configured repeatability sweep records whether density/grammar/collapse repair "
                "holds across seeds and routes remaining dead-air separately"
            ),
        },
        "not_proven": [
        ...
        "next_recommended_issue": (
            "Stage B MIDI-to-solo controlled scale checkpoint training scale density grammar collapse repeatability consolidation"
            if target_qualified and strict_gate_stable
            else "Stage B MIDI-to-solo controlled scale checkpoint training scale density grammar collapse dead-air remaining blocker decision"
            if target_qualified and dead_air_remaining
            else "Stage B MIDI-to-solo controlled scale checkpoint training scale density grammar collapse follow-up decision"
        ),
    }
...
def validate_repeatability_report(
    ...
    *,
    ...
    if require_completed and not bool(readiness.get("density_grammar_collapse_repeatability_probe_completed", False)):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDensityGrammarCollapseRepeatabilityProbeError(
    ...
    return {
        ...
        "density_grammar_collapse_repeatability_target_supported": bool(
            readiness.get("density_grammar_collapse_repeatability_target_supported", False)
        ),
        "strict_gate_stable": bool(readiness.get("strict_gate_stable", False)),
        "dead_air_remaining": bool(readiness.get("dead_air_remaining", False)),
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- density/grammar/collapse repeatability target supported: `{_bool_token(readiness['density_grammar_collapse_repeatability_target_supported'])}`",
        f"- strict gate stable: `{_bool_token(readiness['strict_gate_stable'])}`",
        f"- dead-air remaining: `{_bool_token(readiness['dead_air_remaining'])}`",
        ...
        f"- note-count / grammar / dead-air failure count: `{aggregate['note_count_failure_count']}` / `{aggregate['grammar_failure_count']}` / `{aggregate['dead_air_failure_count']}`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run selected-scale density/grammar/collapse repeatability probe")
    parser.add_argument(
        "--repair_report",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repair_probe/"
        "harness_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repair_probe/"
        "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repair_probe.json",
    )
    parser.add_argument("--checkpoint_dir", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repeatability_probe",
    )
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--doc_path", type=str, default="")
    parser.add_argument("--issue_number", type=int, default=588)
    parser.add_argument("--seeds", type=str, default="47,52,60")
...
def main() -> int:
    ...
    write_json(
        run_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repeatability_probe.json",
        report,
    )
    write_json(
        run_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repeatability_probe_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        run_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_density_grammar_collapse_repeatability_probe.md",
        markdown,
    )
    if args.doc_path:
