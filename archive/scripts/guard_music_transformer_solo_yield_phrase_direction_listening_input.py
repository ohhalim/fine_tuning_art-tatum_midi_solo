"""Guard listening input for phrase-direction repaired solo-yield candidates."""
...
from scripts.build_music_transformer_solo_yield_phrase_direction_listening_package import (  # noqa: E402
...
SCHEMA_VERSION = "music_transformer_solo_yield_phrase_direction_listening_input_guard_v1"
INPUT_SCHEMA_VERSION = "music_transformer_solo_yield_phrase_direction_listening_input_v1"
BOUNDARY = "music_transformer_solo_yield_phrase_direction_repair_listening_input_guard"
NEXT_BOUNDARY_PENDING = "music_transformer_solo_yield_phrase_direction_repair_objective_only_next_decision"
NEXT_BOUNDARY_REVIEWED = "music_transformer_solo_yield_phrase_direction_repair_listening_review_fill"


class SoloYieldPhraseDirectionListeningInputGuardError(ValueError):
    pass
...
def read_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise SoloYieldPhraseDirectionListeningInputGuardError(f"json not found: {path}")
...
def _require_no_quality_claim(readiness: dict[str, Any]) -> None:
    ...
    if claimed:
        raise SoloYieldPhraseDirectionListeningInputGuardError(f"unexpected quality claim: {claimed}")
...
def validate_package(package_report: dict[str, Any]) -> dict[str, Any]:
    if str(package_report.get("schema_version") or "") != SOURCE_PACKAGE_SCHEMA_VERSION:
        raise SoloYieldPhraseDirectionListeningInputGuardError("source package schema required")
    ...
    if candidate_count <= 0:
        raise SoloYieldPhraseDirectionListeningInputGuardError("candidate count required")
    if not bool(readiness.get("listening_package_ready", False)):
        raise SoloYieldPhraseDirectionListeningInputGuardError("source package readiness required")
    if _int(readiness.get("candidate_midi_files_copied")) != candidate_count:
        raise SoloYieldPhraseDirectionListeningInputGuardError("MIDI copy count mismatch")
    if _int(readiness.get("candidate_wav_files_copied")) != candidate_count:
        raise SoloYieldPhraseDirectionListeningInputGuardError("WAV copy count mismatch")
    if not bool(readiness.get("review_input_template_written", False)):
        raise SoloYieldPhraseDirectionListeningInputGuardError("review input template required")
    return {
...
def build_guard_report(
    ...
    *,
    ...
    write_json(output_dir / "phrase_direction_repair_listening_input_guard.json", report)
    write_json(
        output_dir / "phrase_direction_repair_listening_input_guard_summary.json",
        validate_report(report, require_no_quality_claim=True),
    )
    write_text(output_dir / "phrase_direction_repair_listening_input_guard.md", markdown_report(report))
    return report


def validate_report(report: dict[str, Any], *, require_no_quality_claim: bool) -> dict[str, Any]:
    if str(report.get("schema_version") or "") != SCHEMA_VERSION:
        raise SoloYieldPhraseDirectionListeningInputGuardError("schema version mismatch")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Music Transformer Solo Yield Phrase Direction Repair Listening Input Guard",
        "",
        "## Summary",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Guard phrase direction repair listening input")
    parser.add_argument(
        "--package_report",
        type=str,
        default=(
            "outputs/music_transformer_finetune_mvp/solo_yield_phrase_direction_repair_listening_review/"
            "issue_1278_phrase_direction_listening_package/listening_review_package.json"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/music_transformer_finetune_mvp/solo_yield_phrase_direction_repair_listening_input_guard",
    )
