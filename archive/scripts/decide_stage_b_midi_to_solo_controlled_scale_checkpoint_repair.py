"""Decide the next repair target after controlled checkpoint generation failure."""
...
class StageBMidiToSoloControlledScaleCheckpointRepairDecisionError(ValueError):
    pass


BOUNDARY = "stage_b_midi_to_solo_controlled_scale_checkpoint_repair_decision"
NEXT_BOUNDARY = "stage_b_midi_to_solo_controlled_scale_checkpoint_density_collapse_repair_probe"
SCHEMA_VERSION = "stage_b_midi_to_solo_controlled_scale_checkpoint_repair_decision_v1"


def read_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError(f"report missing: {path}")
...
def validate_generation_probe(report: dict[str, Any]) -> dict[str, Any]:
    if str(report.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError(
            "controlled checkpoint generation probe boundary required"
        )
    ...
    if str(decision.get("next_boundary") or "") != BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError(
            "generation probe must route to controlled checkpoint repair decision"
        )
    if not bool(readiness.get("generation_path_executable", False)):
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("generation path must be executable")
    if bool(readiness.get("raw_generation_quality_ready", True)):
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("raw generation quality must not be ready")
    if bool(generation.get("passed_strict_review_gate", True)):
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("strict review gate should fail")
    blocked_claims = [
    ...
    if claimed:
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError(f"unexpected quality claim: {claimed}")
    ...
    postprocess_failure_count = _count_prefixed(strict, "postprocess removal ratio too high:")
    repeated_pair_failure_count = _count_prefixed(strict, "repeated position/pitch pair ratio too high:")
    if _int(generation.get("sample_count")) <= 0:
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("sample count required")
    if note_count_failure_count <= 0:
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("note-count failure evidence required")
    if _int(generation.get("grammar_gate_sample_count")) <= 0:
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("grammar gate evidence required")
...
def build_repair_decision(
    ...
    *,
    ...
    collapse_across_all_samples = evidence["collapse_warning_sample_count"] == evidence["sample_count"]
    ...
    return {
        ...
        "repair_decision": {
            ...
            "reason": (
                "controlled checkpoint generation produced grammar-valid token groups, but all samples failed "
                "MIDI review because note density collapsed after postprocess"
            ),
        },
        "claim_boundary": {
            "boundary": BOUNDARY,
        ...
        "next_recommended_issue": "Stage B MIDI-to-solo controlled checkpoint density collapse repair probe",
    }
...
def validate_repair_decision(
    ...
    *,
    ...
    if expected_boundary and boundary != expected_boundary:
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("unexpected boundary")
    if expected_next_boundary and str(decision.get("next_boundary") or "") != expected_next_boundary:
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("unexpected next boundary")
    if require_repair_target and str(decision.get("selected_target") or "") != "target_density_collapse_postprocess_repair":
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("density/collapse repair target required")
    if not bool(evidence.get("all_samples_note_count_failed", False)):
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("all-sample note-count failure required")
    if not bool(evidence.get("collapse_across_all_samples", False)):
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("all-sample collapse warning required")
    if not bool(evidence.get("postprocess_removal_high", False)):
        raise StageBMidiToSoloControlledScaleCheckpointRepairDecisionError("postprocess removal evidence required")
    ...
    return {
        ...
        "collapse_across_all_samples": bool(evidence.get("collapse_across_all_samples", False)),
        ...
        "training_scale_change_selected": bool(decision.get("training_scale_change_selected", True)),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Decide controlled checkpoint repair target")
    parser.add_argument(
        "--generation_probe",
        type=str,
    ...
    parser.add_argument("--output_root", type=str, default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_repair_decision")
...
def main() -> int:
    ...
    write_json(output_dir / "stage_b_midi_to_solo_controlled_scale_checkpoint_repair_decision.json", report)
    write_json(
        output_dir / "stage_b_midi_to_solo_controlled_scale_checkpoint_repair_decision_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(output_dir / "stage_b_midi_to_solo_controlled_scale_checkpoint_repair_decision.md", markdown)
    if args.doc_path:
