...
from scripts.run_stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_probe import (  # noqa: E402
    BOUNDARY as SOURCE_BOUNDARY,
    NEXT_BOUNDARY as SOURCE_NEXT_BOUNDARY,
)


class StageBMidiToSoloPitchContourChangedRatioRepairAudioError(ValueError):
    pass


BOUNDARY = "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_audio_package"
NEXT_BOUNDARY = "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_listening_review_package"
SCHEMA_VERSION = "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_audio_package_v1"
...
def _require_no_quality_claim(container: dict[str, Any], *, label: str) -> None:
    ...
    if claimed:
        raise StageBMidiToSoloPitchContourChangedRatioRepairAudioError(
...
def validate_source_report(
    ...
    *,
    ...
    if str(report.get("boundary") or readiness.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloPitchContourChangedRatioRepairAudioError(
            "changed-ratio repair probe boundary required"
        )
    if str(decision.get("next_boundary") or "") != SOURCE_NEXT_BOUNDARY:
        raise StageBMidiToSoloPitchContourChangedRatioRepairAudioError(
            "changed-ratio repair probe must route to audio package"
        )
    required_true = [
        "changed_ratio_repair_probe_completed",
        "repaired_ranked_midi_written",
        "changed_ratio_repair_passed",
        "changed_ratio_repair_audio_render_required",
    ]
    missing = [name for name in required_true if not bool(readiness.get(name, False))]
    if missing:
        raise StageBMidiToSoloPitchContourChangedRatioRepairAudioError(
            f"missing changed-ratio repair readiness: {missing}"
        )
    ...
    if _float(summary.get("repaired_max_pitch_changed_ratio")) > _float(
        summary.get("max_pitch_changed_ratio")
    ):
        raise StageBMidiToSoloPitchContourChangedRatioRepairAudioError(
            "repaired pitch changed ratio exceeds target"
        )
    ...
    for row in rows[: int(expected_count)]:
        ...
        if not _path_exists(repaired_midi_path):
            raise StageBMidiToSoloPitchContourChangedRatioRepairAudioError(
                f"changed-ratio repaired MIDI missing: {repaired_midi_path}"
            )
        if not bool(row.get("candidate_repair_passed", False)):
            raise StageBMidiToSoloPitchContourChangedRatioRepairAudioError(
                "candidate changed-ratio repair should pass before audio render"
            )
    ...
    *,
    ...
    return {
        ...
        "summary": {
            ...
            "max_repaired_pitch_changed_ratio": max(pitch_changed_ratios, default=0.0),
            "target_max_pitch_changed_ratio": _float(
                _dict(source_report.get("summary")).get("max_pitch_changed_ratio")
            ),
        ...
        "decision": {
            ...
            "reason": (
                "changed-ratio repaired MIDI rendered to WAV; prepare listening review package next"
            ),
        },
        "not_proven": [
...
def validate_audio_render_report(
    ...
    *,
    ...
    if require_audio_package_completed and not bool(
        boundary.get("changed_ratio_repair_audio_package_completed", False)
    ):
        raise StageBMidiToSoloPitchContourChangedRatioRepairAudioError(
    ...
    return {
        ...
        "changed_ratio_repair_audio_package_completed": bool(
            boundary.get("changed_ratio_repair_audio_package_completed", False)
        ),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Render model-conditioned pitch-contour changed-ratio repair WAV files"
    )
    parser.add_argument("--changed_ratio_repair_probe_report", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_audio_package",
    )
...
def main() -> int:
    ...
    report = build_audio_render_report(
        read_json(Path(args.changed_ratio_repair_probe_report)),
        output_dir=output_dir,
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_audio_package.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_audio_package_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_model_conditioned_pitch_contour_changed_ratio_repair_audio_package.md",
        markdown,
    )
    if args.doc_path:
