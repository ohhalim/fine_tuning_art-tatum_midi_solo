"""Run repeatability probe for selected-scale checkpoint dead-air repair."""
...
class StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
    ValueError
):
    pass
...
PASS_NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "dead_air_repair_repeatability_consolidation"
)
FAIL_NEXT_BOUNDARY = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "dead_air_repeatability_temperature_guard_decision"
)
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "dead_air_repair_repeatability_probe_v1"
)
...
def parse_seeds(raw: str) -> list[int]:
    ...
    if not seeds:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
            "seed list required"
        )
    return seeds
...
def validate_repair_probe(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(report.get("boundary") or readiness.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
            "selected-scale dead-air repair boundary required"
        )
    if str(decision.get("next_boundary") or "") != BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
            "selected-scale dead-air repair must route to repeatability probe"
        )
    if not bool(readiness.get("selected_scale_dead_air_target_qualified", False)):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
            "selected-scale dead-air repair target support required"
        )
    blocked = [
    ...
    if _int(repair.get("note_count_failure_count")) != 0:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
            "source note-count failure must be removed"
        )
    if _int(repair.get("grammar_failure_count")) != 0:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
            "source grammar failure must be removed"
        )
    ...
    if _int(repair.get("collapse_warning_sample_count")) != 0:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
            "source collapse warning must be removed"
        )
    return {
        ...
        "source_collapse_warning_sample_count": _int(
            repair.get("collapse_warning_sample_count")
        ),
...
def build_repeatability_report(
    *,
    ...
    target_qualified = (
        ...
        and _int(aggregate["dead_air_failure_count"]) == 0
    ...
    return {
        ...
        "readiness": {
            ...
            "selected_scale_dead_air_repair_repeatability_target_qualified": bool(
                target_qualified
            ),
        ...
        "decision": {
            ...
            "reason": (
                "selected-scale dead-air repair is measured across seeds; remaining failures "
                "route to a temperature guard decision without changing the quality claim boundary"
            ),
        },
        "not_proven": [
        ...
        "next_recommended_issue": (
            "Stage B MIDI-to-solo controlled scale checkpoint training scale dead-air repair repeatability consolidation"
            if target_qualified
            else "Stage B MIDI-to-solo controlled scale checkpoint training scale dead-air repeatability temperature guard decision"
        ),
    }
...
def validate_repeatability_report(
    ...
    *,
    ...
    if require_completed and not bool(
        readiness.get("dead_air_repair_repeatability_probe_completed", False)
    ):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepairRepeatabilityProbeError(
    ...
    return {
        ...
        "selected_scale_dead_air_repair_repeatability_target_qualified": bool(
            readiness.get("selected_scale_dead_air_repair_repeatability_target_qualified", False)
        ),
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- selected-scale dead-air repair repeatability target qualified: `{_bool_token(readiness['selected_scale_dead_air_repair_repeatability_target_qualified'])}`",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run selected-scale dead-air repair repeatability probe"
    )
    parser.add_argument(
        "--repair_report",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repair_probe/"
        "harness_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repair_probe/"
    ...
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repair_repeatability_probe",
    )
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--doc_path", type=str, default="")
    parser.add_argument("--issue_number", type=int, default=594)
...
def main() -> int:
    ...
    write_json(
        run_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repair_repeatability_probe.json",
        report,
    )
    write_json(
        run_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repair_repeatability_probe_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        run_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repair_repeatability_probe.md",
        markdown,
    )
    if args.doc_path:
