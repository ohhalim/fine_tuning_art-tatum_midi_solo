"""Render chord-tone landing repaired solo-yield MIDI candidates to WAV."""
...
SCHEMA_VERSION = "music_transformer_solo_yield_chord_tone_landing_repair_audio_package_v1"
BOUNDARY = "music_transformer_solo_yield_chord_tone_landing_repair_audio_package"
NEXT_BOUNDARY = "music_transformer_solo_yield_chord_tone_landing_repair_listening_package"
CommandRunner = Callable[[Sequence[str]], subprocess.CompletedProcess[str]]


class SoloYieldChordToneLandingRepairAudioPackageError(ValueError):
    pass
...
def _require_no_quality_claim(report: dict[str, Any]) -> None:
    ...
    claimed = [
        key
        for key in ("musical_quality_claimed", "audio_rendered_quality_claimed", "artist_style_claimed", "production_ready_claimed")
        if bool(readiness.get(key, False))
    ]
    if claimed:
        raise SoloYieldChordToneLandingRepairAudioPackageError(
            f"unexpected quality claim: {claimed}"
        )


def _validate_repair_sweep(report: dict[str, Any]) -> list[dict[str, Any]]:
    if str(report.get("schema_version") or "") != REPAIR_SWEEP_SCHEMA_VERSION:
        raise SoloYieldChordToneLandingRepairAudioPackageError("repair sweep schema required")
    _require_no_quality_claim(report)
    aggregate = _dict(report.get("aggregate"))
    if not bool(aggregate.get("target_supported", False)):
        raise SoloYieldChordToneLandingRepairAudioPackageError("repair target support required")
    rows = [_dict(item) for item in _list(report.get("candidate_repairs"))]
    if not rows:
        raise SoloYieldChordToneLandingRepairAudioPackageError("candidate repairs required")
    for row in rows:
        midi_path = Path(str(row.get("repaired_midi_path") or ""))
        if not midi_path.exists():
            raise SoloYieldChordToneLandingRepairAudioPackageError(f"repaired MIDI missing: {midi_path}")
    return rows


def _build_fallback_repair_sweep(*, output_dir: Path, package_report_path: Path) -> dict[str, Any]:
    ...
    return build_repair_sweep(
        ...
        output_dir=output_dir / "source_repair_sweep",
    )
...
def load_or_build_repair_sweep(
    *,
    ...
    return _build_fallback_repair_sweep(output_dir=output_dir, package_report_path=package_report_path)
...
def build_render_plan(
    ...
    *,
    ...
    if not renderer.exists():
        raise SoloYieldChordToneLandingRepairAudioPackageError(f"renderer not found: {renderer}")
    if not soundfont.exists():
        raise SoloYieldChordToneLandingRepairAudioPackageError(f"soundfont not found: {soundfont}")
    ...
    for row in repairs:
        ...
        wav_path = output_dir / "audio" / f"candidate_{review_index:02d}_{case_label}_chord_tone_landing_repair.wav"
        plan.append(
            {
    ...
    *,
    ...
    report = {
        ...
        "source_repair_sweep": {
            ...
            "repaired_midi_count": _int(_dict(repair_sweep.get("aggregate")).get("repaired_midi_count")),
        ...
        "decision": {
            ...
            "selected_next_target": "chord_tone_landing_repair_listening_package",
            "next_boundary": NEXT_BOUNDARY,
            "critical_user_input_required": False,
            "reason": "chord-tone landing repaired MIDI rendered to WAV for technical listening package",
        },
        "not_proven": [
    ...
    write_json(output_dir / "chord_tone_landing_repair_audio_package.json", report)
    write_json(output_dir / "chord_tone_landing_repair_audio_package_summary.json", validate_report(report))
    write_text(output_dir / "chord_tone_landing_repair_audio_package.md", markdown_report(report))
    return report


def validate_report(report: dict[str, Any], *, min_wav_count: int = 1) -> dict[str, Any]:
    if str(report.get("schema_version") or "") != SCHEMA_VERSION:
        raise SoloYieldChordToneLandingRepairAudioPackageError("schema version mismatch")
    readiness = _dict(report.get("readiness"))
    aggregate = _dict(report.get("aggregate"))
    rendered_count = _int(aggregate.get("rendered_wav_count"))
    if rendered_count < int(min_wav_count):
        raise SoloYieldChordToneLandingRepairAudioPackageError("rendered WAV count below requirement")
    if not bool(readiness.get("audio_package_completed", False)):
        raise SoloYieldChordToneLandingRepairAudioPackageError("audio package completion required")
    if not bool(aggregate.get("technical_wav_validation", False)):
        raise SoloYieldChordToneLandingRepairAudioPackageError("technical WAV validation required")
    claimed = [
        key
        for key in ("audio_rendered_quality_claimed", "musical_quality_claimed", "artist_style_claimed", "production_ready_claimed")
        if bool(readiness.get(key, True))
    ]
    if claimed:
        raise SoloYieldChordToneLandingRepairAudioPackageError(f"unexpected quality claim: {claimed}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Render chord-tone landing repaired MIDI to WAV")
    parser.add_argument(
        "--repair_sweep_report",
        type=str,
        default=(
    ...
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/music_transformer_finetune_mvp/solo_yield_chord_tone_landing_repair_audio",
    )
...
def main() -> int:
    ...
    write_json(output_dir / "chord_tone_landing_repair_audio_package_summary.json", summary)
    if args.doc_path:
