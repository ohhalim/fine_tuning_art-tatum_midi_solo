"""Decide temperature guard after selected-scale dead-air repeatability failure."""
...
class StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepeatabilityTemperatureGuardDecisionError(
    ValueError
):
    pass
...
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "dead_air_repeatability_temperature_guard_decision_v1"
)
...
def validate_repeatability_probe(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if boundary != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepeatabilityTemperatureGuardDecisionError(
            "selected-scale dead-air repeatability boundary required"
        )
    ...
    if bool(
        readiness.get(
            "selected_scale_dead_air_repair_repeatability_target_qualified",
            True,
        )
    ):
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScaleDeadAirRepeatabilityTemperatureGuardDecisionError(
    ...
    *,
    ...
    return {
        ...
        "decision": {
            ...
            "reason": (
                "selected-scale repeatability preserved grammar and seed-level gates, "
                "but all-sample strict validity failed under source temperature"
            ),
        },
        "claim_boundary": {
            "boundary": BOUNDARY,
        ...
        "next_recommended_issue": (
            ...
            "dead-air repeatability temperature guard repair probe"
        ),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Decide selected-scale dead-air repeatability temperature guard"
    )
    parser.add_argument(
        "--repeatability_report",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repair_repeatability_probe/"
        "harness_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repair_repeatability_probe/"
    ...
    parser.add_argument("--issue_number", type=int, default=596)
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repeatability_temperature_guard_decision_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_dead_air_repeatability_temperature_guard_decision.md",
        markdown,
    )
    if args.doc_path:
