"""Summarize a bounded MIDI-to-solo controlled training scale smoke."""
...
from scripts.decide_stage_b_midi_to_solo_training_scale_expansion import (  # noqa: E402
...
class StageBMidiToSoloControlledTrainingScaleSmokeSummaryError(ValueError):
    pass
...
SCHEMA_VERSION = "stage_b_midi_to_solo_controlled_training_scale_smoke_v1"


def read_json(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError(f"report missing: {path}")
...
def validate_decision(decision_report: dict[str, Any]) -> dict[str, Any]:
    if str(decision_report.get("boundary") or "") != DECISION_BOUNDARY:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("training scale expansion decision required")
    ...
    if str(decision.get("next_boundary") or "") != DECISION_NEXT_BOUNDARY:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("decision must route to controlled scale smoke")
    if not bool(readiness.get("controlled_training_scale_smoke_ready", False)):
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("controlled scale smoke readiness required")
    if bool(readiness.get("cloud_or_gpu_spend_required", True)):
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("cloud/GPU spend should not be required")
    return {
...
def validate_training_smoke(training_smoke: dict[str, Any], decision: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(training_smoke.get("boundary") or readiness.get("boundary") or "") != TRAINING_SMOKE_BOUNDARY:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("training scale smoke boundary required")
    ...
    if not bool(readiness.get("training_scale_smoke_passed", False)):
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("training scale smoke should pass")
    if _int(inputs.get("selected_train_records")) != _int(decision.get("selected_train_records")):
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("selected train records mismatch")
    if _int(inputs.get("selected_val_records")) != _int(decision.get("selected_val_records")):
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("selected val records mismatch")
    if _int(config.get("max_sequence")) != _int(decision.get("max_sequence")):
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("max_sequence mismatch")
    if _int(artifacts.get("checkpoint_count")) < 1:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("checkpoint artifact required")
    if _float(training.get("best_validation_loss")) <= 0.0:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("best validation loss required")
    blocked_claims = [
    ...
    if claimed:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError(f"unexpected training claim: {claimed}")
    return {
...
def build_controlled_training_scale_smoke_summary(
    ...
    *,
    ...
    return {
        ...
        "decision": {
            ...
            "reason": "bounded 512/128 max_sequence 160 training smoke completed; next step is checkpoint generation probe",
        },
        "not_proven": [
            "checkpoint_generation_probe_result",
        ...
        "next_recommended_issue": "Stage B MIDI-to-solo controlled scale checkpoint generation probe",
    }


def validate_controlled_training_scale_smoke_summary(
    report: dict[str, Any],
    *,
    ...
    if expected_boundary and boundary != expected_boundary:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("unexpected boundary")
    if expected_next_boundary and str(decision.get("next_boundary") or "") != expected_next_boundary:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("unexpected next boundary")
    if _int(training.get("selected_train_records")) < int(min_train_records):
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("train records below requirement")
    if _int(training.get("selected_val_records")) < int(min_val_records):
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("val records below requirement")
    if require_checkpoint and _int(training.get("checkpoint_count")) < 1:
        raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError("checkpoint required")
    if require_no_quality_claim:
        ...
        if claimed:
            raise StageBMidiToSoloControlledTrainingScaleSmokeSummaryError(f"unexpected quality claim: {claimed}")
    return {
        "boundary": boundary,
        ...
        "checkpoint_generation_probe_ready": bool(readiness.get("checkpoint_generation_probe_ready", False)),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize MIDI-to-solo controlled training scale smoke")
    parser.add_argument(
        "--decision_report",
        type=str,
        default="outputs/stage_b_midi_to_solo_training_scale_expansion_decision/"
        "harness_stage_b_midi_to_solo_training_scale_expansion_decision/"
        "stage_b_midi_to_solo_training_scale_expansion_decision.json",
    )
    parser.add_argument(
        "--training_smoke",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_training_scale_smoke/"
        "harness_stage_b_midi_to_solo_controlled_training_scale_smoke/training_smoke/"
        "controlled_512_128_maxseq160/stage_b_generic_base_training_scale_smoke.json",
    )
    parser.add_argument("--output_root", type=str, default="outputs/stage_b_midi_to_solo_controlled_training_scale_smoke")
...
def main() -> int:
    ...
    write_json(output_dir / "stage_b_midi_to_solo_controlled_training_scale_smoke.json", report)
    write_json(output_dir / "stage_b_midi_to_solo_controlled_training_scale_smoke_validation_summary.json", summary)
    markdown = markdown_report(report)
    write_text(output_dir / "stage_b_midi_to_solo_controlled_training_scale_smoke.md", markdown)
    if args.doc_path:
