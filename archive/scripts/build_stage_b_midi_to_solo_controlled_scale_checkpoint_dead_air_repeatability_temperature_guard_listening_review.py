"""Build pending listening review boundary for controlled temperature-guard WAVs."""
...
from scripts.build_stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_audio_package import (  # noqa: E402
...
class StageBMidiToSoloControlledScaleCheckpointTemperatureGuardListeningReviewError(
    ValueError
):
    pass
...
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_"
    "temperature_guard_listening_review_v1"
)
...
def compact_rendered_candidate(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if not bool(wav_file.get("exists", False)) or not wav_path.exists():
        raise StageBMidiToSoloControlledScaleCheckpointTemperatureGuardListeningReviewError(
...
def validate_audio_package(audio_package: dict[str, Any], expected_count: int) -> list[dict[str, Any]]:
    ...
    if str(boundary.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTemperatureGuardListeningReviewError(
            "temperature guard audio package boundary required"
        )
...
def listening_review_template(candidates: list[dict[str, Any]]) -> str:
    lines = [
        "# Stage B MIDI-to-Solo Controlled Temperature Guard Listening Review Input",
        "",
        "## Review Status",
        "",
        "- reviewer: `pending`",
...
def build_listening_review_report(
    ...
    *,
    ...
    review_input_path = output_dir / "review" / "temperature_guard_listening_review_input.md"
    ...
    return {
        ...
        "decision": {
            ...
            "reason": (
                "temperature guard listening review input remains pending; route to objective-only "
        ...
        "next_recommended_issue": (
            "Stage B MIDI-to-solo controlled checkpoint dead-air temperature guard objective-only next decision"
        ),
    }
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Stage B MIDI-to-Solo Controlled Temperature Guard Listening Review",
        "",
        "## Summary",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build controlled temperature guard listening review boundary"
    )
    parser.add_argument("--audio_package", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_listening_review",
    )
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_listening_review.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_listening_review_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_dead_air_repeatability_temperature_guard_listening_review.md",
        markdown,
    )
    if args.doc_path:
