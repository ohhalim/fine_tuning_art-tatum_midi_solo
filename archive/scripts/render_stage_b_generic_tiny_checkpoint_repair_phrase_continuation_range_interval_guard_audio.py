...
class StageBGenericTinyCheckpointRepairPhraseContinuationRangeIntervalGuardAudioRenderError(ValueError):
    pass
...
def required_review_items(local_audio_render_package: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if str(boundary.get("boundary") or "") != PACKAGE_BOUNDARY:
        raise StageBGenericTinyCheckpointRepairPhraseContinuationRangeIntervalGuardAudioRenderError(
            "unexpected range/interval guard audio render package boundary"
        )
...
def build_audio_render_report(
    ...
    *,
    ...
    return {
        "schema_version": "stage_b_generic_tiny_checkpoint_repair_phrase_continuation_range_interval_guard_local_audio_render_attempt_v1",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Render range/interval guard repair audio")
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_generic_tiny_checkpoint_repair_phrase_continuation_range_interval_guard_local_audio_render_attempt_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_generic_tiny_checkpoint_repair_phrase_continuation_range_interval_guard_local_audio_render_attempt.md",
        markdown,
    )
    if args.doc_path:
