"""Build audio review package for controlled checkpoint postprocess-removal-dead-air-repair candidates."""
...
class StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairAudioPackageError(
    ValueError
):
    pass
...
SCHEMA_VERSION = (
    "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_"
    "postprocess_removal_dead_air_repair_audio_review_package_v1"
)
...
def validate_consolidation_report(report: dict[str, Any], expected_seed_count: int) -> dict[str, Any]:
    ...
    if str(report.get("boundary") or "") != SOURCE_BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairAudioPackageError(
            "postprocess removal dead-air repair consolidation boundary required"
        )
    if str(decision.get("next_boundary") or "") != BOUNDARY:
        raise StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairAudioPackageError(
            "postprocess removal dead-air repair consolidation must route to audio review package"
        )
    ...
    *,
    ...
    for item in candidates:
        wav_path = (
            output_dir
            / "audio"
            / f"postprocess_removal_dead_air_repair_rank_{int(item['rank']):02d}_seed_{int(item['seed'])}.wav"
        )
        plan.append(
            {
                **item,
    ...
    *,
    ...
    return {
        ...
        "decision": {
            ...
            "reason": "objective-supported postprocess removal dead-air repair MIDI candidates rendered to WAV for listening review",
        },
        "not_proven": [
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build controlled postprocess removal dead-air repair audio review package"
    )
    parser.add_argument("--consolidation_report", type=str, required=True)
    parser.add_argument(
        "--output_root",
        type=str,
        default="outputs/stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_audio_review_package",
    )
...
def main() -> int:
    ...
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_audio_review_package.json",
        report,
    )
    write_json(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_audio_review_package_validation_summary.json",
        summary,
    )
    markdown = markdown_report(report)
    write_text(
        output_dir
        / "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_audio_review_package.md",
        markdown,
    )
    if args.doc_path:
