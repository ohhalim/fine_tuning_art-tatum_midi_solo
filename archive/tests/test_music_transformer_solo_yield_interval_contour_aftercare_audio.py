...
from scripts.render_music_transformer_solo_yield_interval_contour_aftercare_audio import (
    NEXT_BOUNDARY,
    SCHEMA_VERSION,
    SoloYieldIntervalContourAftercareAudioPackageError,
    build_audio_package,
    validate_report,
)
...
def repair_sweep(root: Path, *, quality_claim: bool = False) -> dict:
    ...
    for path in midi_paths:
        write_midi(path)
    return {
        "schema_version": INTERVAL_REPAIR_SCHEMA_VERSION,
...
class MusicTransformerSoloYieldIntervalContourAftercareAudioTest(unittest.TestCase):
    def test_renders_interval_repaired_midi_wavs_without_quality_claim(self) -> None:
    ...
    def test_rejects_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            renderer.write_text("#!/bin/sh\n", encoding="utf-8")
            soundfont.write_bytes(b"sf2")
            with self.assertRaises(SoloYieldIntervalContourAftercareAudioPackageError):
                build_audio_package(
