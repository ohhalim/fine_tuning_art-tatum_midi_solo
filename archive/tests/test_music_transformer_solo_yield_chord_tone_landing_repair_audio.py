...
from scripts.render_music_transformer_solo_yield_chord_tone_landing_repair_audio import (
    NEXT_BOUNDARY,
    SCHEMA_VERSION,
    SoloYieldChordToneLandingRepairAudioPackageError,
    build_audio_package,
    validate_report,
)
...
class MusicTransformerSoloYieldChordToneLandingRepairAudioTest(unittest.TestCase):
    def test_renders_repaired_midi_wavs_without_quality_claim(self) -> None:
    ...
    def test_rejects_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            renderer.write_text("#!/bin/sh\n", encoding="utf-8")
            soundfont.write_bytes(b"sf2")
            with self.assertRaises(SoloYieldChordToneLandingRepairAudioPackageError):
                build_audio_package(
