...
from scripts.guard_music_transformer_solo_yield_chord_role_balance_listening_input import (
    BOUNDARY,
    ...
    SoloYieldChordRoleBalanceListeningInputGuardError,
    build_guard_report,
    validate_report,
)
...
class MusicTransformerSoloYieldChordRoleBalanceListeningInputGuardTest(unittest.TestCase):
    ...
    def test_rejects_source_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            with self.assertRaises(SoloYieldChordRoleBalanceListeningInputGuardError):
                build_guard_report(
