...
from scripts.guard_music_transformer_solo_yield_phrase_direction_listening_input import (
...
class MusicTransformerSoloYieldPhraseDirectionObjectiveNextTest(unittest.TestCase):
    def test_selects_chord_role_balance_when_low_chord_tone_is_largest_residual(self) -> None:
        rows = [
            after_profile(chord_tone_ratio=0.48, max_interval=8),
            after_profile(chord_tone_ratio=0.49, max_interval=8),
            after_profile(chord_tone_ratio=0.47, note_count=29),
            after_profile(chord_tone_ratio=0.55),
            after_profile(chord_tone_ratio=0.56),
            after_profile(chord_tone_ratio=0.57, note_count=28),
            after_profile(chord_tone_ratio=0.58),
            after_profile(chord_tone_ratio=0.59),
        ]
        ...
        self.assertEqual(summary["midi_low_chord_tone_ratio_count"], 3)
    ...
    def test_selects_density_aftercare_when_low_note_count_is_only_residual(self) -> None:
        rows = [
            after_profile(chord_tone_ratio=0.55, note_count=29),
            after_profile(chord_tone_ratio=0.56, note_count=28),
            after_profile(chord_tone_ratio=0.57, note_count=32),
        ]
        with tempfile.TemporaryDirectory() as temp_dir:
            report = build_decision(repair_sweep(rows), guard_report(), output_dir=Path(temp_dir))
        summary = validate_report(report, require_no_quality_claim=True)
    ...
    def test_validated_input_routes_to_listening_review_fill(self) -> None:
        rows = [
            after_profile(chord_tone_ratio=0.45),
            after_profile(chord_tone_ratio=0.49),
        ]
        with tempfile.TemporaryDirectory() as temp_dir:
