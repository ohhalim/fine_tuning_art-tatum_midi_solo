...
from scripts.build_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_listening_review import (
...
from scripts.decide_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_objective_next import (
    FINAL_BOUNDARY,
    NEXT_BOUNDARY,
    StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextError,
...
def listening_review(*, validated_input: bool = False, quality_claim: bool = False) -> dict:
    return {
        "schema_version": "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_listening_review_v1",
        "boundary": LISTENING_BOUNDARY,
        "review_candidates": [
            {
                "rank": rank,
                "midi_path": f"outputs/postprocess_removal_dead_air_repair/midi/rank_{rank:02d}.mid",
                "wav_path": f"outputs/postprocess_removal_dead_air_repair/audio/rank_{rank:02d}.wav",
        ...
        "decision": {
            "next_boundary": "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_objective_only_next_decision",
...
def consolidation(
    *,
    ...
    return {
        "schema_version": "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_consolidation_v1",
        ...
        "consolidation_result": {
            "objective_midi_support": strict_count == sample_count
        ...
        "decision": {
            "next_boundary": "stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_audio_review_package",
            "critical_user_input_required": False,
        },
    }


class StageBMidiToSoloControlledScaleCheckpointPostprocessRemovalDeadAirRepairObjectiveNextTest(
    unittest.TestCase
):
    def test_closes_objective_path_and_routes_training_scale_decision(self) -> None:
        report = build_objective_next_decision_report(
            listening_review(),
            consolidation(),
            output_dir=Path("outputs/postprocess_removal_dead_air_repair_objective_next"),
        )
        ...
        self.assertEqual(summary["avg_postprocess_removal_ratio"], 0.2176)
        self.assertEqual(summary["max_postprocess_removal_ratio"], 0.2917)
        self.assertEqual(summary["target_avg_postprocess_removal_ratio"], 0.3)
        self.assertTrue(summary["objective_postprocess_removal_dead_air_repair_path_supported"])
