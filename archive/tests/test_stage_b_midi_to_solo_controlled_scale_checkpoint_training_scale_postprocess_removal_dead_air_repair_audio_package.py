...
from scripts.build_stage_b_midi_to_solo_controlled_scale_checkpoint_training_scale_postprocess_removal_dead_air_repair_audio_package import (
    BOUNDARY,
    NEXT_BOUNDARY,
    StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairAudioPackageError,
  ...
  "samples": [
    {
      ...
      "metrics": {"dead_air_ratio": 0.45, "unique_pitch_count": 9, "note_count": 14, "duration_sec": 3.7},
    ...
    },
    {
      ...
      "strict_valid": true,
      "grammar_gate_passed": true,
      "metrics": {"dead_air_ratio": 0.35, "unique_pitch_count": 12, "note_count": 16, "duration_sec": 3.8},
...
class StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairAudioPackageTest(
    unittest.TestCase
):
    def test_selects_seed_candidate_by_postprocess_then_dead_air(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            report_path = generation_report(Path(temp_dir), 44)
            candidate = select_seed_candidate(report_path)

        self.assertEqual(candidate["sample_index"], 1)
    ...
    def test_rejects_unsupported_consolidation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            renderer.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
            soundfont.write_bytes(b"sf2")
            with self.assertRaises(StageBMidiToSoloControlledScaleCheckpointTrainingScalePostprocessRemovalDeadAirRepairAudioPackageError):
                build_audio_package_report(
