...
from scripts.guard_music_transformer_solo_yield_phrase_direction_listening_input import (
    ...
    SoloYieldPhraseDirectionListeningInputGuardError,
    build_guard_report,
    validate_report,
)
...
class MusicTransformerSoloYieldPhraseDirectionListeningInputGuardTest(unittest.TestCase):
    ...
    def test_rejects_source_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            with self.assertRaises(SoloYieldPhraseDirectionListeningInputGuardError):
                build_guard_report(
