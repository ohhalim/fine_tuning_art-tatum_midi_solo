...
from scripts.fill_stage_b_generic_tiny_checkpoint_repair_phrase_continuation_range_interval_guard_sparse_phrase_user_listening_review import (
    AUDIO_BOUNDARY,
    NEXT_BOUNDARY,
    REVIEW_BOUNDARY_REJECT_ALL,
    StageBGenericTinyCheckpointRepairPhraseContinuationRangeIntervalGuardSparsePhraseUserListeningReviewError,
...
def rendered_item(rank: int) -> dict:
    return {
        "review_rank": rank,
        "interval_cap": 5 if rank < 3 else 9,
        "sample_seed": 80 + rank,
        "sample_index": rank,
        "source_midi_path": f"sparse_sample_{rank}.mid",
        "wav_file": {
            "path": f"sparse_sample_{rank}.wav",
            "exists": True,
            "duration_seconds": 6.7 + rank / 10,
            "sample_rate": 44100,
            "sha256": str(rank) * 64,
        },
    }
...
def audio_render_report(*, quality_claimed: bool = False) -> dict:
    return {
        "schema_version": (
            ...
            "sparse_phrase_local_audio_render_attempt_v1"
        ),
...
class StageBGenericTinyCheckpointRepairPhraseContinuationRangeIntervalGuardSparsePhraseUserListeningReviewTest(
    unittest.TestCase
):
    def test_records_sparse_phrase_reject_all_without_keep_or_quality_claim(self) -> None:
        report = build_user_listening_review(
            ...
            assessment="all sparse phrase candidates rejected by single-user listening review",
            notes="sparse phrase objective gate was insufficient for listening acceptance",
        )
