...
from scripts.build_stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_listening_review import (
    BOUNDARY,
    NEXT_BOUNDARY,
    StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeListeningReviewError,
...
def audio_package(root: Path, *, quality_claim: bool = False, technical_valid: bool = True) -> dict:
    ...
    for rank in range(1, 4):
        wav_path = root / f"contour_rank_{rank:02d}.wav"
        fake_wav(wav_path)
        rendered.append(
            {
                "rank": rank,
                "midi_path": str(root / f"contour_rank_{rank:02d}.mid"),
    ...
    return {
        ...
        "audio_package_boundary": {
            ...
            "source_boundary": "stage_b_midi_to_solo_model_direct_jazz_phrase_vocabulary_contour_phrase_shape_repair",
            "candidate_count": 3,
...
class StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeListeningReviewTest(unittest.TestCase):
    ...
    def test_rejects_upstream_quality_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            with self.assertRaises(StageBMidiToSoloModelDirectJazzPhraseVocabularyContourPhraseShapeListeningReviewError):
                build_listening_review_report(
